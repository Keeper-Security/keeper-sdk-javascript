...
function summarize(result: ApplyMembershipResult): void {
    for (const folder of result.folders) {
        const label = folder.sharedFolderUid ? `${folder.path} (${folder.sharedFolderUid})` : folder.path
        if (folder.success) {
            logger.info(
            ...
            logger.error(`${label}: FAILED${folder.message ? ` - ${folder.message}` : ''}`)
        }
    }
    for (const team of result.teamMembership) {
        const label = team.teamUid ? `${team.name} (${team.teamUid})` : team.name
        if (team.success) {
            logger.info(`Team "${label}" membership: +${team.added}/-${team.removed}`)
        } else {
            logger.error(`Team "${label}" membership: FAILED${team.message ? ` - ${team.message}` : ''}`)
        }
    }
...
async function applyMembershipCommand() {
    ...
    try {
        const inputPath =
            (await prompt('Input JSON file path (default membership.json): ')).trim() || 'membership.json'
        ...
        try {
            ...
            logger.error(`Could not read "${inputPath}": ${extractErrorMessage(err)}`)
            process.exitCode = 1
            return
        }
        ...
        try {
            ...
            logger.error(`Could not parse "${inputPath}" as JSON: ${extractErrorMessage(err)}`)
            process.exitCode = 1
            return
        }
        ...
        summarize(result)
        if (!result.success) {
            logger.error('Some shared folders or teams could not be fully synced. See details above.')
            process.exitCode = 1
        }
    } catch (err) {
