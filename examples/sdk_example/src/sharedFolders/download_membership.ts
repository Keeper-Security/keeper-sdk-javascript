...
import type { DownloadMembershipOptions, MembershipData } from '@keeper-security/keeper-sdk-javascript'
...
async function downloadMembershipCommand() {
    ...
    try {
        const outputPath =
            (await prompt('Output JSON file path (default membership.json): ')).trim() || 'membership.json'
        ...
        logger.info(`Wrote ${data.shared_folders.length} shared folder(s) to "${outputPath}".`)
        if (data.teams) {
