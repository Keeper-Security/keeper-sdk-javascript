...
async function listRotationSchedulesExample() {
    ...
    try {
        ...
        const rotations = [...result.rotations].sort((a, b) => {
            const aMissingConfig = a.pamConfigDisplay === '[No Config Found]' ? 1 : 0
            const bMissingConfig = b.pamConfigDisplay === '[No Config Found]' ? 1 : 0
            return aMissingConfig - bMissingConfig || a.recordTitle.localeCompare(b.recordTitle)
        })

        logger.info('')
        logger.info(vault.formatRotationSchedulesOutput({ ...result, rotations }, options))
        logger.info('')
    } catch (err) {
