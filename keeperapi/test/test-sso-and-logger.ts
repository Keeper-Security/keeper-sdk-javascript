...
import {Records, ServiceLogger} from '../src/proto'
...
async function cleanVault(user?: UserInfo) {
    try {
...
async function printMSPVault() {
    ...
    try {
        ...
        await auth.login(MIKE_VAULT_LOGIN_1.account, MIKE_VAULT_LOGIN_1.password);
...
async function downloadSharedFiles(fileRecordUid1: string, fileRecordUid2: string) {
    let auth1 = await login({ account: "saldoukhov@gmail.com", password: "111111"});
