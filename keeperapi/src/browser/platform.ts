...
export const browserPlatform: Platform = class {
    ...
    static async loadKeyBytes(keyId: string, storage?: KeyStorage): Promise<Uint8Array> {
        ...
        const keyBytes = storage ? await storage.getKeyBytes(keyId) : undefined;
        if (!keyBytes) {
    ...
    static async unwrapKeys(keys: UnwrapKeyMap, storage?: KeyStorage): Promise<void> {
        if (workerPool) {
            try {
                ...
                // Import keys
                await Promise.all(
                    Object.entries(unwrappedKeys).map(async ([keyId, keyBytes]) => {
                        try {
        ...
        await Promise.all(
            Object.values(keys).map(async (task) => {
    ...
    static async unwrapKey(
        ...
        unwrappedKeyType: UnwrappedKeyType,
        storage?: KeyStorage,
        canExport?: boolean
    ): Promise<void> {
    ...
    static async unwrapAesKey(
        ...
        const cbcKey = await crypto.subtle.unwrapKey(
            'raw',
            wrappedKey,
            unwrappingKey,
            algoParams,
    ...
    static async publicEncryptECWithHKDF(
    ...
    static async mainPublicEncryptEC(data: Uint8Array, key: Uint8Array, id?: Uint8Array, useHKDF?: boolean) {
        const ephemeralKeyPair = await crypto.subtle.generateKey({ name: 'ECDH', namedCurve: 'P-256' }, true, [
            'deriveBits',
        ]);
    ...
    static async deriveSharedSecretKey(
        ...
        if (!useHKDF) {
            ...
            const symmetricKey = await this.hkdf(
                new Uint8Array(),
    ...
    static async privateDecryptECWebCrypto(
        data: Uint8Array,
        privateKey: CryptoKey,
        id?: Uint8Array,
        useHKDF?: boolean
    ...
    static async privateSign(data: Uint8Array, key: string): Promise<Uint8Array> {
        let _key = await crypto.subtle.importKey('pkcs8', browserPlatform.base64ToBytes(key), 'RSA-PSS', true, [
            'sign',
        ]);
    ...
    static async aesWrapKey(
        keyId: string,
        wrappingKeyId: string,
        encryptionType: Exclude<CryptoKeyType, 'ecc'>,
        storage?: KeyStorage
    ): Promise<Uint8Array> {
    ...
    static async aesGcmImportKey(keyBytes: Uint8Array, extractable: boolean): Promise<CryptoKey> {
        return crypto.subtle.importKey('raw', keyBytes, 'AES-GCM', extractable, [
            'decrypt',
            'encrypt',
            'unwrapKey',
            'wrapKey',
    ...
    static async aesCbcImportKey(keyBytes: Uint8Array, extractable: boolean): Promise<CryptoKey> {
        return crypto.subtle.importKey('raw', keyBytes, 'AES-CBC', extractable, [
            'decrypt',
            'encrypt',
            'unwrapKey',
            'wrapKey',
    ...
    static fileUpload(url: string, uploadParameters: { [key: string]: string }, data: Blob): Promise<any> {
