...
describe('Sync Down', () => {
    ...
    describe('Owned Records', () => {
        ...
        it('saves breach watch data and security score data if a record contains a password', async () => {
            ...
            const { recordUid, recordKey, record, decryptedSecurityScoreDataData } =
                await syncDownResponseBuilder.addRecord(decryptedRecordData)
            const recordUidStr = webSafe64FromBytes(recordUid)
            const userFolderRecord: Vault.IUserFolderRecord = {
                recordUid,
                revision: Date.now(),
    ...
    describe('Directly-Shared Records', () => {
        ...
        it('saves the record data when the direct-shared record is updated by another user or the user', async () => {
            ...
            const recordUidStr = webSafe64FromBytes(recordUid)
            const recordMetadata: Vault.IRecordMetaData = {
                // recordMetadata is returned only the share permission is updated
                recordUid,
                recordKey,
                owner: false,
                canEdit: true,
    ...
    describe('User Folders', () => {
        it.each([
            ...
            const userFolder: Vault.IUserFolder = {
                ...
                userFolderKey,
                data: await platform.aesCbcEncrypt(
                    platform.stringToBytes(JSON.stringify(folderData)),
                    decryptedUserFolderKey,
                    true
                ),
                parentUid: new Uint8Array([]),
            }
            syncDownResponseBuilder.addUserFolder(userFolder)
            mockSyncDownCommand.mockResolvedValue(syncDownResponseBuilder.build())
            await syncDown({
                auth,
                storage,
    ...
    describe('Shared Folders', () => {
        it('saves the shared folder data when a new shared folder is created by the user', async () => {
            ...
            const { sharedFolderUid, sharedFolder } = await syncDownResponseBuilder.addSharedFolder(
                sharedFolderData,
                syncDownUser,
                {
        ...
        it("saves the shared folder data when the user's team is added to the folder", async () => {
            ...
            const encryptedTeamPrivateKey = await platform.aesCbcEncrypt(
                decryptedTeamPrivateKeyPair.privateKey,
                decryptedTeamKey,
                true
            )
            const { sharedFolderUid, sharedFolder, sharedFolderKey } = await syncDownResponseBuilder.addSharedFolder(
                sharedFolderData,
                anotherUserA,
                {
                    defaultCanEdit: false,
        ...
        it('saves the record data when a new record is created in a shared folder', async () => {
            ...
            const { sharedFolderUid, sharedFolder, decryptedSharedFolderKey } =
            ...
            const sharedFolderUidStr = webSafe64FromBytes(sharedFolderUid)
            const { recordKey, recordUid } = await syncDownResponseBuilder.addRecord(
                decryptedRecordData,
      ...
      */
            ...
            const { sharedFolderUid: sharedFolderUidA, sharedFolder: sharedFolderA } =
                await syncDownResponseBuilder.addSharedFolder(sharedFolderDataA, syncDownUser, {})
            const {
                sharedFolderUid: sharedFolderUidB,
                sharedFolder: sharedFolderB,
                decryptedSharedFolderKey: decryptedSharedFolderKeyB,
            } = await syncDownResponseBuilder.addSharedFolder(sharedFolderDataB, syncDownUser, {})
            ...
            const { recordUid, recordKey, record } = await syncDownResponseBuilder.addRecord(
                decryptedRecordData,
                decryptedSharedFolderKeyB
            )
            const recordUidStr = webSafe64FromBytes(recordUid)
            const removedSharedFolderFolderRecord: Vault.ISharedFolderFolderRecord = {
                recordUid,
    ...
    describe('Shared-Folder Folders', () => {
        it.each([
            ...
            const { sharedFolderFolderUid, sharedFolderFolder } = await syncDownResponseBuilder.addSharedFolderFolder(
                decryptedFolderData,
                sharedFolderUid,
                decryptedSharedFolderKey
            )
       ...
       */
            ...
            const { sharedFolderFolderUid: sharedFolderFolderBUid, sharedFolderFolder } =
                await syncDownResponseBuilder.addSharedFolderFolder(
                    decryptedFolderData,
                    sharedFolderUid,
                    decryptedSharedFolderKey,
                    sharedFolderFolderAUid
                )
        ...
        it('saves the record data when a new record data is added to a shared-folder folder', async () => {
            ...
            const sharedFolderUidStr = webSafe64FromBytes(sharedFolderUid)
            const { sharedFolderFolderUid, sharedFolderFolder } = await syncDownResponseBuilder.addSharedFolderFolder(
                sharedFolderFolderData,
                sharedFolderUid,
                decryptedSharedFolderKey
            )
            const sharedFolderFolderUidStr = webSafe64FromBytes(sharedFolderFolderUid)
            const { recordUid, record, recordKey } = await syncDownResponseBuilder.addRecord(
                recordData,
                decryptedSharedFolderKey
            )
            const recordUidStr = webSafe64FromBytes(recordUid)
    ...
    describe('Linked Records', () => {
        describe('V4 Linked Records - File Attachments', () => {
            it('saves the record data when a new linked record is added to a record', async () => {
                ...
                const { recordUid, record, decryptedRecordKey, recordKey } =
                    await syncDownResponseBuilder.addRecord(recordData)
                const recordUidStr = webSafe64FromBytes(recordUid)
                const { linkedRecordUid, linkedRecordKey, linkedRecord } =
                    await syncDownResponseBuilder.addLinkedRecord(linkedRecordData, 4, decryptedRecordKey)
            ...
            it('saves the record data when a linked record is updated, attached to another record', async () => {
                ...
                const { recordUid, record, decryptedRecordKey, recordKey } =
                    await syncDownResponseBuilder.addRecord(recordData)
                const recordUidStr = webSafe64FromBytes(recordUid)
                const { linkedRecordUid, linkedRecord, linkedRecordKey } =
                    await syncDownResponseBuilder.addLinkedRecord(linkedRecordData, 4, decryptedRecordKey)
                const linkedRecordUidStr = webSafe64FromBytes(linkedRecordUid)
                await platform.unwrapKey(recordKey, recordUidStr, 'data', 'gcm', 'aes')
        ...
        describe('V3 Linked Records - Credit Cards / Addresses', () => {
            it('saves the record data when a new linked record is added to a record', async () => {
                ...
                const recordUidStr = webSafe64FromBytes(recordUid)
                const { linkedRecordUid, linkedRecord, linkedRecordKey } =
                    await syncDownResponseBuilder.addLinkedRecord(linkedRecordData, 3, decryptedRecordKey)
                const linkedRecordUidStr = webSafe64FromBytes(linkedRecordUid)
                const recordLink: Vault.IRecordLink = {
                    parentRecordUid: recordUid,
                    childRecordUid: linkedRecordUid,
            ...
            it('saves the record data when a linked record is updated, attached to another record', async () => {
                ...
                const { linkedRecordUid, linkedRecordKey, linkedRecord } =
                    await syncDownResponseBuilder.addLinkedRecord(linkedRecordData, 3, auth.dataKey!)
                const linkedRecordUidStr = webSafe64FromBytes(linkedRecordUid)
