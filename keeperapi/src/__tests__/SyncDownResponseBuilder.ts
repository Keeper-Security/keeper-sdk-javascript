...
export class SyncDownResponseBuilder {
    ...
    async addRecord(decryptedRecordData: DecryptedRecordData, encryptionKey?: Uint8Array) {
        ...
        const recordKey = await this.platform.aesGcmEncrypt(
            decryptedRecordKey,
            encryptionKey ? encryptionKey : this.auth.dataKey!
        )
        ...
        if (!!passwordFieldValue) {
            ...
            this.data.securityScoreData?.push({
                ...
                data: await platform.aesGcmEncrypt(
                    platform.stringToBytes(JSON.stringify(decryptedSecurityScoreDataData)),
                    decryptedRecordKey
                ),
                revision: record.revision,
            })
    ...
    async addSharedFolder(
        ...
        if (!options?.encryptionKey) {
            sharedFolderKey = await platform.aesCbcEncrypt(
                decryptedSharedFolderKey,
                options?.encryptionKey ? options?.encryptionKey : this.auth.dataKey!,
                true
            )
        } else {
            // normally when a shared folder is shared to a team
            sharedFolderKey = platform.publicEncrypt(
                decryptedSharedFolderKey,
                platform.bytesToBase64(options.encryptionKey)
            )
        }
        const sharedFolder: Vault.ISharedFolder = {
            ...
            keyType: options?.encryptionKey
                ? Records.RecordKeyType.NO_KEY
                : Records.RecordKeyType.ENCRYPTED_BY_DATA_KEY,
            revision: Date.now(),
            name: await platform.aesCbcEncrypt(
                platform.stringToBytes(decryptedSharedFolderData.name),
                decryptedSharedFolderKey,
                true
            ),
            data: await platform.aesCbcEncrypt(
                platform.stringToBytes(JSON.stringify(decryptedSharedFolderData)),
                decryptedSharedFolderKey,
                true
            ),
            ...permissionData,
    ...
    async addSharedFolderFolder(
        ...
        const sharedFolderFolderKey = await platform.aesCbcEncrypt(
            decryptedSharedFolderFolderKey,
            decryptedSharedFolderKey,
            true
        )
        const sharedFolderFolder: Vault.ISharedFolderFolder = {
            ...
            revision: Date.now(),
            data: await platform.aesCbcEncrypt(
                platform.stringToBytes(JSON.stringify(decryptedSharedFolderFolderData)),
                decryptedSharedFolderFolderKey,
                true
            ),
    ...
    async addLinkedRecord(data: DecryptedRecordData, version: 3 | 4, encryptionKey: Uint8Array) {
        ...
        const linkedRecordData = await this.platform.aesGcmEncrypt(
            this.platform.stringToBytes(JSON.stringify(data)),
            decryptedLinkedRecordKey
        )
