...
export const nodePlatform: Platform = class {
    ...
    static async mainPublicEncryptEC(
        data: Uint8Array,
        key: Uint8Array,
        id?: Uint8Array,
        useHKDF?: boolean
        ...
        const symmetricKey = !useHKDF
            ? crypto.createHash('SHA256').update(sharedSecretCombined).digest()
            : Buffer.from(hkdfSync('sha256', sharedSecret, new Uint8Array(), id ?? Buffer.from([]), 32))
    ...
    static privateSign(data: Uint8Array, key: string): Promise<Uint8Array> {
        return Promise.resolve(crypto.createSign('SHA256').update(data).sign(key))
    }
    ...
    static async aesCbcEncrypt(data: Uint8Array, key: Uint8Array, usePadding: boolean): Promise<Uint8Array> {
        ...
        let cipher = crypto.createCipheriv('aes-256-cbc', key, iv).setAutoPadding(usePadding)
        let encrypted = Buffer.concat([cipher.update(data), cipher.final()])
    ...
    static async aesCbcDecrypt(data: Uint8Array, key: Uint8Array, usePadding: boolean): Promise<Uint8Array> {
        ...
        let cipher = crypto.createDecipheriv('aes-256-cbc', key, iv).setAutoPadding(usePadding)
    ...
    static deriveKeyV2(
        ...
        const bytes = crypto.pbkdf2Sync(
            Buffer.of(...Buffer.from(domain), ...nodePlatform.unWrapPassword(password)),
            saltBytes,
            iterations,
            64,
            'SHA512'
    ...
    static fileUpload(url: string, uploadParameters: { [key: string]: string }, data: Uint8Array): Promise<any> {
    ...
    private static fetchData(res, resolve) {
        ...
        res.on('data', (data) => {
            retVal.data = retVal.data ? Buffer.concat([retVal.data, data]) : data
        })
        res.on('end', () => {
            resolve(retVal)
        })
