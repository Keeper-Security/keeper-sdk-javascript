...
export async function loadKeeperConfig(
  preloaded?: KeeperJsonConfig,
): Promise<KeeperJsonConfig> {
...
export async function resolveServer(
  username?: string,
  preloadedConfig?: KeeperJsonConfig,
): Promise<string> {
...
async function tryPersistentLogin(
  host: string,
  username: string,
): Promise<KeeperVault | null> {
...
async function syncVault(vault: KeeperVault): Promise<KeeperVault> {
  ...
  logger.info(
    `Vault synced. ${vault.getSummary().recordCount} records loaded.\n`,
  );
  return vault;
}
