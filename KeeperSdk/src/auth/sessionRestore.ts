...
function decodeBytes(
  ...
  if (!trimmed) {
    if (required) {
      throw new KeeperSdkError(
        `restore-session: missing required field: ${label}`,
  ...
  try {
    ...
    throw new KeeperSdkError(
      `restore-session: invalid base64 for ${label}: ${msg}`,
...
function parseUserType(value: number | string): UserType {
  ...
  if (s === "cloud_sso" || s === "cloudsso" || s === "1")
    return UserTypeValues.cloudSso;
  if (s === "onsite_sso" || s === "onsitesso" || s === "2")
    return UserTypeValues.onsiteSso;
  throw new KeeperSdkError(
    `restore-session: unknown userType: ${value}`,
    ResultCodes.INVALID_CREDENTIALS,
  );
}

function parseSessionTokenType(
  value: number | string,
): Authentication.SessionTokenType {
  const n = typeof value === "number" ? value : Number(value);
  if (!Number.isFinite(n)) {
    throw new KeeperSdkError(
      `restore-session: invalid sessionTokenType: ${value}`,
      ResultCodes.INVALID_CREDENTIALS,
    );
  }
...
export function validateSessionRestoreInput(
  input: Partial<SessionRestoreInput>,
): SessionRestoreInput {
...
export function toSessionParams(input: SessionRestoreInput): SessionParams {
  const enterprisePublicKey = decodeBytes(
    "enterprisePublicKey",
    input.enterprisePublicKey,
    false,
  );
  const enterpriseEccPublicKey = decodeBytes(
    "enterpriseEccPublicKey",
    input.enterpriseEccPublicKey,
    false,
  );
  ...
  return {
    ...
    messageSessionUid: decodeBytes(
      "messageSessionUid",
      input.messageSessionUid,
      true,
    )!,
...
export function sessionRestoreFromJson(json: string): SessionRestoreInput {
  ...
  if (typeof parsed !== "object" || parsed === null) {
    throw new KeeperSdkError(
      "restore-session: JSON must be an object",
      ResultCodes.INVALID_CREDENTIALS,
    );
  }
