...
export class SessionManager implements SessionStorage {
  ...
  constructor(configDirOrLoader?: string | ConfigLoader) {
    if (
      typeof configDirOrLoader === "string" ||
      configDirOrLoader === undefined
    ) {
      this.configLoader = new FileConfigLoader(
        configDirOrLoader as string | undefined,
      );
    } else {
  ...
  public async getCloneCode(
    ...
    const device = await this.findDeviceInKeeperConfig(username);
    if (device) {
      const serverInfo = device.serverInfo.find(
        (entry) => entry.server === hostStr,
      );
      if (serverInfo) {
  ...
  private async updateKeeperConfigCloneCode(
    host: string,
    username: string,
    cloneCode: Uint8Array,
  ): Promise<void> {
    try {
      ...
      if (
        parsed.user?.toLowerCase() === username.toLowerCase() &&
        server === host
      ) {
      ...
      const user = (parsed.users || []).find(
        (configUser) =>
          configUser.user?.toLowerCase() === username.toLowerCase(),
      );
      if (user?.last_device?.device_token) {
        const device = (parsed.devices || []).find(
          (configDevice) =>
            configDevice.device_token === user.last_device.device_token,
        );
        if (device?.server_info) {
          const serverInfo = device.server_info.find(
            (entry) => entry.server === host,
          );
          if (serverInfo) {
      ...
      logger.warn(
        "Failed to update keeper config clone code:",
  ...
  public async saveSessionParameters(
  ...
  private async findDeviceInKeeperConfig(
    username: string,
  ): Promise<Nullable<ResolvedDevice>> {
  ...
  private async lookupDeviceInKeeperConfig(
    ...
    if (keeperConfig.users && keeperConfig.devices) {
      ...
      if (user?.last_device?.device_token) {
        ...
        const device = keeperConfig.devices.find(
          (configDevice) => configDevice.device_token === deviceTokenStr,
        );
