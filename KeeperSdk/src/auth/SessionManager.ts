...
export class SessionManager implements SessionStorage {
  ...
  private async lookupDeviceInKeeperConfig(
    ...
    if (kc.users && kc.devices) {
      const user = kc.users.find(
        (u) => u.user?.toLowerCase() === normalizedUsername,
      );
      if (user?.last_device?.device_token) {
        const deviceTokenStr = user.last_device.device_token;
        const device = kc.devices.find(
          (d) => d.device_token === deviceTokenStr,
        );
        if (device?.private_key) {
          return {
            ...
            serverInfo: (device.server_info || []).filter(
              (si): si is Required<ConfigurationServerConfig> =>
    ...
    if (
      kc.device_token &&
      kc.private_key &&
      kc.user?.toLowerCase() === normalizedUsername
    ) {
