...
export class ConsoleAuthUI implements AuthUI3 {
  ...
  private static twoFactorChannelName(
    channelType: Authentication.TwoFactorChannelType,
  ): string {
    switch (channelType) {
  ...
  private static async waitWithCancel(
    timeoutMs: number,
    cancel?: Promise<void>,
  ): Promise<void> {
    if (!cancel) {
  ...
  public async waitForDeviceApproval(
    ...
    try {
      ...
      if (selected.validateCode) {
        ...
        logger.info(
          "Approval request sent. Waiting for approval on your other device...",
        );
  ...
  public async waitForTwoFactorCode(
    ...
    try {
      ...
      channels.forEach((ch, i) => {
        const name = ConsoleAuthUI.twoFactorChannelName(
      ...
      const selected = channels[idx];
      const name = ConsoleAuthUI.twoFactorChannelName(
        selected.channel.channelType!,
      );

      if (selected.availablePushes && selected.availablePushes.length > 0) {
        const pushChoice = (
          await rl.question(`Send push notification for ${name}? (y/n): `)
        ).trim();
        if (pushChoice.toLowerCase() === "y" && selected.sendPush) {
          ...
          await ConsoleAuthUI.waitWithCancel(
            AuthDefaults.APPROVAL_TIMEOUT_MS,
            cancel,
          );
          return true;
        }
      ...
      await ConsoleAuthUI.waitWithCancel(
        AuthDefaults.CODE_VALIDATION_DELAY_MS,
        cancel,
      );
      return true;
    } catch (e) {
