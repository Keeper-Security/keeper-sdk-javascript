...
export function validateRoleName(name: string): void {
  if (!name.trim()) {
    throw new KeeperSdkError(
      ...
      ResultCodes.ROLE_NAME_EMPTY,
    );
  }
}

export function buildRolesByLowerName(
  roles: EnterpriseRole[],
): Map<string, EnterpriseRole[]> {
...
export function resolveExistingRoles(
  roles: EnterpriseRole[],
  identifiers: string[],
): EnterpriseRole[] {
  const byId = new Map<number, EnterpriseRole>(
    roles.map((role) => [role.role_id, role]),
  );
...
export function applyDecryptedRoleNames(
  roles: EnterpriseRole[],
...
export function nodePathOrFallback(
...
function roleHasEnforcement(
  ...
  return links.some(
    (link) =>
      link.role_id === roleId &&
      normalizeEnforcementKey(link.enforcement_type) === enforcement,
  );
}
