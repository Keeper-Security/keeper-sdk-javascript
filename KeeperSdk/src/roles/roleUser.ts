...
const membershipKey = (roleId: number, userId: number): string =>
  `${roleId}:${userId}`;
...
async function loadRoleUserContext(
  ...
  if (userIdentifiers.length === 0) {
    throw new KeeperSdkError(
      "No users specified.",
      ResultCodes.NO_USERS_FOR_ROLE_OP,
    );
  }
...
function buildMembershipSet(
  roleUsers: EnterpriseRoleUserLink[] | undefined,
): Set<string> {
...
function buildItemBase(
  user: EnterpriseUser,
  role: EnterpriseRole,
): Omit<RoleUserItemResult, "status"> {
  return {
...
function finalizeResult(items: RoleUserItemResult[]): RoleUserResult {
  ...
  for (const item of items) {
    if (item.status === RoleUserStatus.Added || item.status === RoleUserStatus.Removed)
      succeeded++;
...
export function formatRoleUserResult(
  result: RoleUserResult,
