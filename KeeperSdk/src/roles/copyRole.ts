...
export async function copyRoles(
  auth: Auth,
  input: CopyRoleInput,
  ...
  if (!sourceIdentifier) {
    throw new KeeperSdkError(
      ...
      ResultCodes.ROLE_SOURCE_REQUIRED,
    );
  }
  ...
  const sourceRoleName =
    (sourceRole.displayName || "").trim() || String(sourceRole.role_id);
  ...
  const { copied: enforcementsCopied, failed: enforcementsFailed } =
    await copyEnforcements(
      auth,
      sourceRole.role_id,
      newRoleId,
  ...
  if (input.clone === true) {
    const userResult = await copyRoleUsers(
      auth,
      sourceRole.role_id,
      newRoleId,
      response.role_users || [],
...
async function copyRoleTeams(
  ...
  try {
    ...
    const message = createInMessage(
      payload,
      "enterprise/role_team_add",
