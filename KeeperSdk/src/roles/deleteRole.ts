...
const DELETE_ROLE_INCLUDES: EnterpriseDataInclude[] = [
...
export async function deleteRoles(
  ...
  if (identifiers.length === 0) {
    throw new KeeperSdkError(
      "No roles to delete.",
      ResultCodes.NO_ROLES_TO_DELETE,
    );
  }
...
export function formatDeleteRoleResult(
...
export function renderDeleteRoleAsciiTable(
  table: FormattedDeleteRoleTable,
): string {
...
async function sendRoleDelete(auth: Auth, roleId: number): Promise<void> {
  const response = await auth.executeRestCommand(
    roleDeleteCommand({ role_id: roleId }),
  );
  assertCommandSucceeded(
    response,
    `role_delete failed for role_id=${roleId}`,
    ResultCodes.ROLE_DELETE_FAILED,
  );
}

function finalizeResult(items: DeleteRoleItemResult[]): DeleteRoleResult {
  const deleted = items.filter(
    (item) => item.status === DeleteRoleStatus.Deleted,
  ).length;
  const failed = items.filter(
    (item) => item.status === DeleteRoleStatus.Failed,
  ).length;
