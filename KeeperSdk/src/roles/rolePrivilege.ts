...
const ROLE_PRIVILEGE_TABLE_HEADERS = [
  "#",
  "Status",
  "Action",
  "Privilege",
...
export async function changeRolePrivileges(
  ...
  if (input.node === undefined || input.node === null || input.node === "") {
    throw new KeeperSdkError(
      "A managed node is required.",
      ResultCodes.NO_NODES_FOR_ROLE_OP,
    );
  }

  const addPrivileges = normalizeIdentifiers(input.add ?? []).map((p) =>
    p.toLowerCase(),
  );
  const removePrivileges = normalizeIdentifiers(input.remove ?? []).map((p) =>
    p.toLowerCase(),
  );
  if (addPrivileges.length === 0 && removePrivileges.length === 0) {
    throw new KeeperSdkError(
      "No privileges specified. Use add and/or remove.",
  ...
  for (const privilege of addPrivileges) {
    try {
      ...
      const response = await auth.executeRestCommand(
        managedNodePrivilegeAddCommand(payload),
      );
      assertCommandSucceeded(
        response,
...
function finalizeResult(
  ...
  for (const item of items) {
    if (
      item.status === RolePrivilegeStatus.Added ||
      item.status === RolePrivilegeStatus.Removed
    )
      succeeded++;
...
export function formatChangeRolePrivilegesResult(
...
export function renderChangeRolePrivilegesAsciiTable(
