...
function buildRoleUserMap(
  links: EnterpriseRoleUserLink[],
...
function buildTeamNameMap(teams: EnterpriseTeamRecord[]): Map<string, string> {
  ...
  for (const team of teams) {
    if (team.team_uid)
      map.set(team.team_uid, (team.name || team.team_uid).trim());
  }
  return map;
}
