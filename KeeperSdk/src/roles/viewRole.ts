...
export async function viewRole(
  ...
  const roleTeams = buildRoleTeams(
    response.role_teams || [],
    response.teams || [],
    role.role_id,
  );
  ...
  const enforcements = buildEnforcements(
...
export function formatRoleView(
  ...
  if (view.role_users && view.role_users.length > 0) {
    mainRows.push({
      ...
      id: verbose
        ? view.role_users.map((u) => u.enterprise_user_id)
        : undefined,
    });
  }
  ...
  const managedNodeTable = buildFormattedManagedNodeTable(
    view.managed_nodes ?? [],
  );
...
function resolveRole(
  ...
  if (!trimmed) {
    throw new KeeperSdkError(
      "Role name or ID is required.",
      ResultCodes.ROLE_REQUIRED,
    );
  }
...
function buildManagedNodes(
  ...
  for (const link of managedNodeLinks) {
    ...
    if (!nodeName && node && !node.parent_id && enterpriseName)
      nodeName = enterpriseName;
    if (!nodeName) nodeName = String(link.managed_node_id);
    const nodePrivileges = (
      privilegesByNode.get(link.managed_node_id) ?? []
    ).sort();
    result.push({
...
function renderMainSection(table: FormattedRoleViewTable): string {
  ...
  const labelWidth = mainRows.reduce(
  ...
  const columnWidths = hasIdColumn
    ? [labelWidth, valueWidth, idWidth]
    : [labelWidth, valueWidth];
  const lines: string[] = [];
  for (const row of expandedRows) {
    const lineCount = Math.max(
      row.valueLines.length,
      hasIdColumn ? row.idLines.length : 0,
      1,
    );
...
function renderManagedNodesSection(
  table: FormattedManagedNodePrivilegeTable,
): string {
