...
const ADD_ROLE_INCLUDES: EnterpriseDataInclude[] = [
...
export type AddRoleConfirm = (
  prompt: AddRoleConflictPrompt,
...
export async function addRoles(
  ...
  if (parentNeedsNameLookup(input.parent ?? null))
...
export function formatAddRoleResult(
  result: AddRoleResult,
): FormattedAddRoleTable {
  return {
    headers: [...ROLE_TABLE_HEADERS],
    rows: buildRoleResultRows(
      result.items,
      (item) => item.message || (item.skipReason ?? ""),
    ),
...
async function allocateRoleId(auth: Auth): Promise<number> {
  const response = await auth.executeRestCommand(
    enterpriseAllocateIdsCommand({ number_requested: 1 }),
  );
  if (!response.base_id) {
    throw new KeeperSdkError(
...
async function sendRoleAdd(
  ...
  assertCommandSucceeded(
    response,
    `role_add failed for role_id=${payload.role_id}`,
    ResultCodes.ROLE_ADD_FAILED,
  );
}
...
async function confirmCrossNode(
  ...
  return (
    (await confirm({
      ...
      existingRoleName:
        (existing.displayName || "").trim() || String(existing.role_id),
...
function finalizeResult(
  ...
  const created = items.filter(
    (item) => item.status === AddRoleStatus.Created,
  ).length;
  const skipped = items.filter(
    (item) => item.status === AddRoleStatus.Skipped,
  ).length;
  const failed = items.filter(
    (item) => item.status === AddRoleStatus.Failed,
  ).length;
  return {
