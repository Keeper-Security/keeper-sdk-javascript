...
export async function updateRoles(
  ...
  if (identifiers.length === 0) {
    throw new KeeperSdkError(
      "No roles to update.",
      ResultCodes.NO_ROLES_TO_UPDATE,
    );
  }
  ...
  if (enforcements.length > 0)
    includes.push(EnterpriseDataInclude.RoleEnforcements);
  ...
  const needsTreeKey =
    Boolean(newName) || resolvedRoles.some((role) => !role.encrypted_data);
  ...
  for (const role of resolvedRoles) {
    ...
    try {
      const payload: RoleEditRequest = {
        ...
        encrypted_data: await resolveEncryptedData(
          role,
          newName,
          currentName,
          treeKey,
...
export function formatUpdateRoleResult(
...
export function renderUpdateRoleAsciiTable(
  table: FormattedUpdateRoleTable,
): string {
...
async function resolveEncryptedData(
  ...
  return encryptObjectForStorage(
    { displayname: newName || currentName },
    treeKey,
  );
}

async function sendRoleUpdate(
  auth: Auth,
...
function finalizeResult(items: UpdateRoleItemResult[]): UpdateRoleResult {
  const updated = items.filter(
    (item) => item.status === UpdateRoleStatus.Updated,
  ).length;
  const failed = items.filter(
    (item) => item.status === UpdateRoleStatus.Failed,
  ).length;
