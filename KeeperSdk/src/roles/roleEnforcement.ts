...
export function formatSetRoleEnforcementsResult(
...
export function renderRoleEnforcementAsciiTable(
...
function finalizeResult(
  ...
  const applied = items.filter(
    (item) => item.status === RoleEnforcementStatus.Applied,
  ).length;
  const failed = items.filter(
    (item) => item.status === RoleEnforcementStatus.Failed,
  ).length;
