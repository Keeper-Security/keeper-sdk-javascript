...
function hasNodeArgument(nodeIdOrName: EditGatewayInput['nodeIdOrName']): nodeIdOrName is string | number {
    return (
...
export async function editGateway(auth: Auth, input: EditGatewayInput): Promise<EditGatewayResult> {
    ...
    const nodeId = hasNode ? await resolveEnterpriseNodeId(auth, input.nodeIdOrName) : previousNodeId
