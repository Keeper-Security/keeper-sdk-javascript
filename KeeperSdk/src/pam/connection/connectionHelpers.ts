...
export function resolveConnectionRecord(storage: InMemoryStorage, identifier: string): DRecord | undefined {
    ...
    if (matches.length > 1)
        throw new KeeperSdkError(`Multiple records match "${identifier}".`, ResultCodes.PAM_CONNECTION_RECORD_AMBIGUOUS)
    return matches[0]
}
...
export function validateConnectionInput(input: PamConnectionEditInput): void {
    if (!input.record?.trim())
        throw new KeeperSdkError('Record UID, title, or path is required.', ResultCodes.PAM_CONNECTION_RECORD_REQUIRED)
    if (input.protocol && !(PAM_CONNECTION_PROTOCOLS as readonly string[]).includes(input.protocol.toLowerCase())) {
        throw new KeeperSdkError(
            `Unsupported connection protocol "${input.protocol}".`,
            ResultCodes.PAM_CONNECTION_PROTOCOL_INVALID
        )
    }
    if (
        input.connectionsOverridePort != null &&
        (!Number.isInteger(input.connectionsOverridePort) ||
            input.connectionsOverridePort < 1 ||
            input.connectionsOverridePort > 65535)
    ) {
        throw new KeeperSdkError(
            'Connection override port must be an integer from 1 to 65535.',
...
export function applyResourceRecordSettings(
    ...
    if (!seedExists) {
        ...
        const target = ['pamMachine', 'pamDatabase', 'pamDirectory', 'pamRemoteBrowser'].includes(data.type)
            ? data.fields
            : data.custom
