...
async function persist(
    ...
    record: DRecord,
    data: ReturnType<typeof rbiData>
): Promise<boolean> {
    if (isNestedShareRecord(storage, record.uid)) {
        const result = await updateNestedShareRecord(storage, auth, {
            ...
            title: data.title,
            notes: data.notes,
            fieldEntries: data.fields,
            customEntries: data.custom,
        })
...
export async function editPamRbi(
    auth: Auth,
    storage: InMemoryStorage,
    input: PamRbiEditInput
): Promise<PamRbiEditResult> {
    ...
    if (getRecordType(record) !== PAM_RBI_RECORD_TYPE)
    ...
    if (input.autofillCredentials) {
        ...
        if (!credential || (credentialType !== 'login' && credentialType !== 'pamUser')) {
            throw new KeeperSdkError(
                'RBI autofill credentials must reference a login or pamUser record.',
                ResultCodes.PAM_RBI_RECORD_INVALID
            )
        }
    ...
    const rbi =
        normalizedInput.remoteBrowserIsolation === 'on'
            ? true
            : normalizedInput.remoteBrowserIsolation === 'off'
              ? false
              : normalizedInput.remoteBrowserIsolation === 'default'
                ? null
                : undefined
    const recording =
        normalizedInput.connectionsRecording === 'on'
            ? true
            : normalizedInput.connectionsRecording === 'off'
              ? false
              : normalizedInput.connectionsRecording === 'default'
                ? null
                : undefined
