...
export function resolveRbiRecord(storage: InMemoryStorage, identifier: string): DRecord | undefined {
    ...
    const matches = storage
        .getRecords()
        .filter((record) => getRecordTitle(record).toLowerCase() === identifier.toLowerCase())
    if (matches.length > 1)
        throw new KeeperSdkError(`Multiple records match "${identifier}".`, ResultCodes.PAM_RBI_RECORD_AMBIGUOUS)
    return matches[0]
}

export function validateRbiInput(input: PamRbiEditInput): void {
    if (!input.record?.trim())
        throw new KeeperSdkError('Record parameter is required.', ResultCodes.PAM_RBI_RECORD_REQUIRED)
    ...
    for (const value of [input.audioChannels, input.audioSampleRate]) {
        if (value != null && (!Number.isInteger(value) || value < 1))
            throw new KeeperSdkError('Audio values must be positive integers.', ResultCodes.PAM_RBI_AUDIO_INVALID)
    }
}
...
export function rbiData(record: DRecord): { type: string; title: string; fields: any[]; custom: any[]; notes: string } {
    ...
    return {
        ...
        title: data.title || getRecordTitle(record),
        fields: structuredClone(data.fields || []),
        custom: structuredClone(data.custom || []),
        notes: data.notes || '',
    }
}

export function updateRbiSettings(
    record: DRecord,
    input: PamRbiEditInput
): { data: ReturnType<typeof rbiData>; changed: boolean } {
    const data = rbiData(record)
    let field = data.fields.find((entry) => entry.type === 'pamRemoteBrowserSettings')
    if (!field) {
        field = { type: 'pamRemoteBrowserSettings', value: [structuredClone(PAM_RBI_DEFAULT_SETTINGS)] }
    ...
    const strings: Array<[string[] | undefined, string]> = [
        [input.allowedUrls, 'allowedUrlPatterns'],
        [input.allowedResourceUrls, 'allowedResourceUrlPatterns'],
        [input.autofillTargets, 'autofillConfiguration'],
    ]
    for (const [values, key] of strings)
        if (values !== undefined) {
            connection[key] = values.join('\n')
            changed = true
        }
    if (input.autofillCredentials) {
        connection.httpCredentialsUid = input.autofillCredentials
        changed = true
    }
    const ints: Array<[number | undefined, string]> = [
        [input.audioChannels, 'audioChannels'],
        [input.audioBitDepth, 'audioBps'],
        [input.audioSampleRate, 'audioSampleRate'],
    ]
    for (const [value, key] of ints)
        if (value !== undefined) {
            connection[key] = value
            changed = true
        }
    if (
        !data.fields.some((entry) => entry.type === 'trafficEncryptionSeed') &&
        !data.custom.some((entry) => entry.type === 'trafficEncryptionSeed')
    ) {
        data.fields.push({ type: 'trafficEncryptionSeed', value: [webSafe64FromBytes(generateEncryptionKey())] })
        changed = true
    }
    return { data, changed }
}
...
export function rbiRecordUidBytes(uid: string): Uint8Array {
