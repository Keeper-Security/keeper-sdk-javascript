...
export async function runAuditReport(
  ...
  if ((SUMMARY_REPORT_TYPES as readonly string[]).includes(reportType)) {
    return runSummaryReport(
      ...
      options,
      hasAram,
      reportType as AuditSummaryReportType,
    );
  }

  throw new KeeperSdkError(
    `Unsupported report type "${reportType}".`,
    ResultCodes.AUDIT_INVALID_REPORT_TYPE,
  );
}
...
async function runDimensionReport(
  ...
  const rows = (await loadDimension(auth, column)).map((row) =>
    dimensionCells(row, fields),
  );
...
async function runRawReport(
  ...
  if (!hasAram) {
    ...
    limit =
      requested != null && requested > 0
        ? Math.min(requested, AUDIT_NO_ARAM_RAW_LIMIT)
        : AUDIT_NO_ARAM_RAW_LIMIT;
  ...
  const templates =
    reportFormat === AuditReportFormat.Message
      ? await loadSyslogTemplates(auth)
...
async function runSummaryReport(
  ...
  if (!hasAram) {
    throw new KeeperSdkError(
      "Audit Reporting addon is not enabled.",
      ResultCodes.AUDIT_REPORTING_NOT_ENABLED,
    );
  }
  ...
  if (
    typeof limit === "number" &&
    (limit < 0 || limit > AUDIT_SUMMARY_MAX_LIMIT)
  ) {
    throw new KeeperSdkError(
      `Invalid "limit" value: ${limit}`,
      ResultCodes.AUDIT_INVALID_LIMIT,
    );
  }
  if (!options.columns?.length) {
    throw new KeeperSdkError(
      '"columns" parameter cannot be empty.',
      ResultCodes.AUDIT_COLUMNS_REQUIRED,
    );
  }

  const aggregates = options.aggregates?.length
    ? options.aggregates
    : [AuditAggregate.Occurrences];
  ...
  const fields = [
    ...aggregates,
    ...(reportType !== "span" ? ["created"] : []),
    ...options.columns,
  ];
  const rows = events.map((event) =>
    fields.map((field) =>
      formatFieldValue(field, getAuditEventField(event, field), reportType),
    ),
  );
  return {
    reportType,
...
async function fetchRawEvents(
  ...
  if (isPaginated && typeof workingFilter?.created === "string") {
    if (
      (CREATED_PRESETS as readonly string[]).includes(workingFilter.created)
    ) {
      workingFilter.created = expandCreatedPreset(
        workingFilter.created as CreatedPreset,
      );
    }
  }
  ...
  while (!done) {
    ...
    assertSucceeded(
      response,
      "get_audit_event_reports failed",
      ResultCodes.AUDIT_REPORT_FAILED,
    );

    let pageEvents = (response.audit_event_overview_report_rows ??
      []) as AuditEventOverviewReportRow[];
    ...
    if ((limit > 0 && eventsReturned >= limit) || pageEvents.length === 0)
      break;
  }
...
async function fetchSummaryEvents(
  ...
  return (response.audit_event_overview_report_rows ??
    []) as AuditEventOverviewReportRow[];
}
...
async function resolveFilter(
  ...
  if (filter.geoLocation || filter.ipAddress) {
    ...
    if (filter.geoLocation) {
      ...
      if (!country) {
        throw new KeeperSdkError(
          '"geoLocation" filter misses country.',
  ...
  if (filter.deviceType) {
    ...
    if (versionFilter.size === 0) {
      throw new KeeperSdkError(
        '"deviceType" filter matched no events.',
        ResultCodes.AUDIT_INVALID_FILTER,
      );
    }
...
async function fetchDimensionRows(
  ...
  assertSucceeded(
    response,
    "get_audit_event_dimensions failed",
    ResultCodes.AUDIT_DIMENSION_FAILED,
  );
...
function buildVirtualDimension(
  dimension: string,
  sourceRows: AuditDimensionRow[],
): AuditDimensionRow[] {
  if (dimension === "geo_location") {
    ...
    return Array.from(geoMap.values()).map((entry) => ({
      ...entry,
      ip_count: Array.isArray(entry.ip_addresses)
        ? entry.ip_addresses.length
        : 0,
    }));
  }
  ...
  if (dimension === "device_type") {
    ...
    for (const row of sourceRows) {
      ...
      if (existing)
        (existing.version_ids as number[]).push(versionRow.version_id);
      else {
...
function getAuditEventField(
  event: AuditEventOverviewReportRow,
  field: string,
): unknown {
...
function eventMessage(
  ...
  while (true) {
    ...
    info =
      info.slice(0, match.index) +
      value +
      info.slice(match.index + match[0].length);
  }
  return info;
}
...
function serializeFilter(
  ...
  if (filter.eventType !== undefined)
    payload.audit_event_type = filter.eventType;
  if (filter.keeperVersion !== undefined)
    payload.keeper_version = filter.keeperVersion;
  ...
  if (filter.ipAddress !== undefined) {
    payload.ip_address = Array.isArray(filter.ipAddress)
      ? filter.ipAddress
      : [filter.ipAddress];
  }
  if (filter.recordUid !== undefined) payload.record_uid = filter.recordUid;
  if (filter.sharedFolderUid !== undefined)
    payload.shared_folder_uid = filter.sharedFolderUid;
  if (filter.parentId !== undefined) payload.parent_id = filter.parentId;

  return Object.keys(payload).length > 0 ? payload : undefined;
}

function createdCriteria(
  criteria: CreatedFilterCriteria,
): AuditReportFilterPayload["created"] {
...
function advanceCreatedFilter(
  ...
  const criteria: CreatedFilterCriteria =
    next.created &&
    typeof next.created === "object" &&
    !Array.isArray(next.created)
      ? { ...next.created }
      : {};
...
function parseBetweenCreatedFilter(
  trimmed: string,
): CreatedFilterCriteria | null {
...
function parseCreatedFilter(value: string): CreatedFilterCriteria {
  ...
  throw new KeeperSdkError(
    `Invalid created filter value "${value}".`,
    ResultCodes.AUDIT_INVALID_CREATED_FILTER,
  );
}
...
async function resolveHasAram(
  ...
  try {
    const response = await auth.executeRestCommand(
      getEnterpriseDataCommand({ include: ["licenses"] }),
    );
    assertSucceeded(
      response,
      "get_enterprise_data failed",
      ResultCodes.AUDIT_LICENSE_CHECK_FAILED,
    );
    return hasEnterpriseAuditReporting(
      response.licenses as EnterpriseAuditLicense[] | undefined,
    );
  } catch {
    return false;
  }
...
function hasEnterpriseAuditReporting(
  licenses: EnterpriseAuditLicense[] | undefined,
): boolean {
  ...
  for (const license of licenses) {
    ...
    for (const addon of license.add_ons || []) {
      ...
      if ((addon.name || "").toLowerCase() === "enterprise_audit_and_reporting")
        return true;
    }
  }
  return false;
...
function asStrOrInt(
  ...
  throw new KeeperSdkError(
    `Invalid "${propertyName}" filter value: ${value}`,
    ResultCodes.AUDIT_INVALID_FILTER,
  );
}
