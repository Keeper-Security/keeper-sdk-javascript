...
export async function runPasswordReport(
  ...
  const targetRecords = await resolveTargetRecords(
    storage,
    session,
    options.folder,
  );
...
function isSupportedRecordVersion(
  version: number,
): version is SupportedRecordVersion {
...
function validatePasswordPolicy(policy: PasswordPolicy): void {
  const hasConstraint = POLICY_SUMMARY_LABELS.some(
    ({ key }) => policy[key] > 0,
  );
  if (!hasConstraint) {
...
function offsetChar(
  text: string,
  compare: (prev: string, current: string) => number,
): number[] {
  if (!text) return [];
...
function getWorstBreachWatchStatus(
...
function getBreachWatchPasswordStatus(
  storage: InMemoryStorage,
  recordUid: string,
  password: string,
): string {
  ...
  if (Array.isArray(passwords) && passwords.length > 0) {
    ...
    const statuses = passwords
      ...
      .filter(
        (status): status is number | string => status != null && status !== "",
      );
...
function buildBreachWatchPasswordCountMap(
  storage: InMemoryStorage,
): Map<string, number> {
  const counts = new Map<string, number>();
  for (const bwRecord of storage.getAll<DBWRecord>("bw_record")) {
    const passwords = (bwRecord.data as BreachWatchRecordData | undefined)
      ?.passwords;
...
async function collectRecordUidsInFolderTree(
  storage: InMemoryStorage,
...
async function resolveTargetRecords(
  ...
  const targetUids = new Set(
    await collectRecordUidsInFolderTree(storage, resolved.folderUid),
  );
  return records.filter((record) => targetUids.has(record.uid));
}

function buildPasswordCountMap(
  records: readonly DRecord[],
): Map<string, number> {
  const counts = new Map<string, number>();
...
function applyVerboseFields(
  ...
  const reuseCount =
    breachWatchPasswordCounts.get(password) ??
    passwordCounts.get(password) ??
    0;
  if (reuseCount > 1) {
