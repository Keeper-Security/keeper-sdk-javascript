...
const TARGET_AUDIT_CONFIG: Record<TargetUserStatus, TargetAuditConfig> = {
  ...
  [TargetUserStatus.Locked]: {
    ...
    pickCandidates: (users) =>
      users.filter((user) => formatUserStatus(user) === "Locked"),
    identity: (user) => user.username,
  },
...
export async function runActionReport(
  ...
  const daysSince =
    options.daysSince ??
    ACTION_DEFAULT_DAYS_BY_TARGET[target] ??
    ACTION_DEFAULT_DAYS;
  ...
  const rows = entries.map((entry) =>
    columns.map((column) => actionReportEntryCell(entry, column)),
  );
...
async function queryUsersWithRecentEvents(
  ...
  for (const batch of chunkArray(identities, ACTION_USERNAME_BATCH_SIZE)) {
    ...
    assertSucceeded(
      ...
      ResultCodes.ACTION_REPORT_FAILED,
    );
...
function buildActionEntryContext(
  data: GetEnterpriseDataResponse,
): ActionEntryContext {
  ...
  const teamNames = new Map(
    (data.teams || []).map((team) => [team.team_uid, team.name]),
  );
  const roleNames = new Map(
    (data.roles || []).map((role) => [
      role.role_id,
      role.displayName || String(role.role_id),
    ]),
  );
...
function buildUserTeamNames(
  ...
  return new Map(
    [...map.entries()].map(([id, names]) => [id, [...names].sort()]),
  );
}
...
function buildUserRoleNames(
  ...
  return new Map(
    [...map.entries()].map(([id, roleIds]) => [
      id,
      [...roleIds]
        .map((roleId) => roleNames.get(roleId) || String(roleId))
        .sort(),
    ]),
  );
}
    ...
    switch (applyAction) {
      ...
      case AdminAction.Delete: {
        ...
        const deleted = result.items.filter(
          (item) => item.status === DeleteUserStatus.Deleted,
        ).length;
        return {
...
function resolveNodeFilter(
  ...
  if (!rootNode) {
    ...
    const matches = nodes.filter(
      (node) => (node.displayName || "").trim().toLowerCase() === lowered,
    );
    if (matches.length === 1) rootNode = matches[0];
    else if (matches.length > 1) {
      throw new KeeperSdkError(
        `Multiple nodes match "${trimmed}".`,
        ResultCodes.ACTION_REPORT_NODE_NOT_UNIQUE,
      );
    }
  }
  if (!rootNode) {
    throw new KeeperSdkError(
      `Node "${trimmed}" was not found.`,
      ResultCodes.ACTION_REPORT_NODE_NOT_FOUND,
    );
  }
...
function resolveActionReportColumns(
  input: ActionReportOptions["columns"],
): ActionReportColumn[] {
  const defaultColumns: ActionReportColumn[] = [
...
function actionReportEntryCell(
  entry: ActionReportEntry,
  column: ActionReportColumn,
): string {
  switch (column) {
