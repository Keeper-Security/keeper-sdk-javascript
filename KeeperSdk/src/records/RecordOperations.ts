...
function getPaddedJsonBytes(data: Record<string, any>): Uint8Array {
  ...
  const paddedLength =
    Math.ceil(Math.max(MIN_RECORD_PAD_BYTES, json.length) / PAD_BLOCK_SIZE) *
    PAD_BLOCK_SIZE;
...
function buildLegacyPasswordExtra(
...
export async function addRecord(
  ...
  if (input.version === 3 && !input.data.type?.trim()) {
    throw new KeeperSdkError(
      "Record type is required for v3 records.",
      "missing_record_type",
    );
  }
  if (input.version === 2) {
...
async function addPasswordRecord(
  ...
  const encryptedExtra = await platform.aesCbcEncrypt(
    extraBytes,
    recordKey,
    true,
  );
  const encryptedRecordKey = await platform.aesCbcEncrypt(
    recordKey,
    auth.dataKey!,
    true,
  );
...
async function addTypedRecord(
  ...
  const encryptedRecordKey = await platform.aesGcmEncrypt(
    recordKey,
    auth.dataKey!,
  );
...
export async function deleteRecord(
  ...
  const preDeleteRequest = { objects };
  ...
  if (!token) {
    return {
      ...
      success: false,
      message:
        preDeleteResponse?.message ||
        preDeleteResponse?.result_code ||
        "pre_delete failed: no token",
    };
  }
  ...
  try {
    await storage.delete(VaultObjectKind.Record, recordUid);
    for (const src of folderLinks) {
      if (src.uid) {
        await storage.removeDependencies({ [src.uid]: new Set([recordUid]) });
      }
    }
  } catch (err) {
    logger.debug(
      `Failed to purge local record ${recordUid}:`,
  ...
  for (const entry of rawHistory) {
    ...
    if (entry.data) {
      try {
        ...
        logger.debug(
          `Failed to decrypt history revision ${entry.revision}:`,
          extractErrorMessage(err),
        );
        decryptedData = null;
      }
...
function resolveFolder(uid: string, storage: InMemoryStorage): FolderInfo {
  ...
  const sfFolder = storage.getByUid<DSharedFolderFolder>(
...
async function findAllRecordFolderLinks(
  ...
  for (const kind of folderKinds) {
    for (const folder of storage.getAll<
      DUserFolder | DSharedFolder | DSharedFolderFolder
    >(kind)) {
      const dependencies = (await storage.getDependencies(folder.uid)) || [];
      if (
        dependencies.some(
          (dependency) =>
            dependency.kind === VaultObjectKind.Record &&
            dependency.uid === recordUid,
        )
      ) {
        const info = resolveFolder(folder.uid, storage);
        const key = `${info.folderType}:${info.uid}`;
        if (!seen.has(key)) {
          seen.add(key);
          links.push(info);
        }
  ...
  if (sharedFolderRecord) {
    const info = resolveFolder(sharedFolderRecord.sharedFolderUid, storage);
    const key = `${info.folderType}:${info.uid}`;
    if (!seen.has(key)) {
      seen.add(key);
      links.push(info);
  ...
  if (links.length === 0) {
    links.push(resolveFolder("", storage));
  }

  return links;
}

async function findRecordSourceFolder(
  recordUid: string,
  storage: InMemoryStorage,
): Promise<string> {
  const links = await findAllRecordFolderLinks(recordUid, storage);
  return links[0]?.uid ?? "";
}
...
export async function moveRecord(
  ...
  const srcUid =
    input.srcFolderUid !== undefined
      ? input.srcFolderUid
      : await findRecordSourceFolder(recordUid, storage);
