...
export async function addUsers(
  ...
  const rawEmails = [
    ...new Map(
      normalizeEmailInputs(input.emails).map(
        (e) => [e.toLowerCase(), e] as const,
      ),
    ).values(),
  ];
...
async function allocateEnterpriseId(auth: Auth): Promise<number> {
  ...
  if (!isNumber(response.base_id) || response.base_id === 0) {
    throw new KeeperSdkError(
      "Failed to allocate enterprise user ID.",
...
async function sendReinvite(
  ...
  assertCommandSuccess(
    response,
    `${REINVITE_COMMAND} failed for user_id=${enterpriseUserId}`,
  );
}
...
function buildExistingByEmail(
  users: EnterpriseUser[],
): Map<string, EnterpriseUser> {
