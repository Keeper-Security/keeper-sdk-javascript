...
async function removeAlias(
  ...
  const exists =
    alias === user.username ||
    existingAliases.some((a) => a.username === alias);
...
async function sendAddAlias(
  ...
  for (const status of response.status || []) {
    if (status.status && status.status !== "success") {
      throw new KeeperSdkError(
        `Add alias failed: ${status.status}`,
        ResultCodes.USER_ALIAS_FAILED,
      );
    }
  }
...
async function sendSetPrimaryAlias(
  auth: Auth,
  enterpriseUserId: number,
  alias: string,
): Promise<void> {
  await auth.executeRestAction(
    enterpriseUserSetPrimaryAliasMessage({ enterpriseUserId, alias }),
  );
}

async function sendDeleteAlias(
  auth: Auth,
  enterpriseUserId: number,
  alias: string,
): Promise<void> {
  await auth.executeRestAction(
    enterpriseUserDeleteAliasMessage({ enterpriseUserId, alias }),
  );
}
