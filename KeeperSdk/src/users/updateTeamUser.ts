import { Enterprise, teamEnterpriseUserUpdateCommand, type Auth } from '@keeper-security/keeperapi'
...
const UPDATE_TEAM_USER_TABLE_HEADERS = ['#', 'Status', 'User Email', 'User ID', 'Team Name', 'Team UID', 'Detail']
...
function buildMembershipSet(teamUsers: EnterpriseTeamUserLink[] | undefined): Set<string> {
...
export async function updateUsersOnTeams(
    ...
    if (input.hideSharedFolders === undefined || input.hideSharedFolders === null) {
        throw new KeeperSdkError('hideSharedFolders (on/off) is required.', ResultCodes.HIDE_SHARED_FOLDERS_REQUIRED)
    }

    const userType = input.hideSharedFolders ? Enterprise.TeamUserType.ADMIN_ONLY : Enterprise.TeamUserType.USER

    const enterpriseData = new EnterpriseDataManager(auth)
    const response = await enterpriseData.getData(UPDATE_TEAM_USER_INCLUDES)

    const teams: ResolvedTeam[] = resolveExistingTeams(response.teams || [], teamIdentifiers).map((team) => ({
        team_uid: team.team_uid,
        name: team.name,
    }))
    const users: EnterpriseUser[] = resolveExistingUsers(response.users || [], emails)
...
function finalizeResult(items: UpdateTeamUserItemResult[]): UpdateUsersOnTeamsResult {
    const updated = items.filter((item) => item.status === UpdateTeamUserStatus.Updated).length
    const skipped = items.filter((item) => item.status === UpdateTeamUserStatus.Skipped).length
    const failed = items.filter((item) => item.status === UpdateTeamUserStatus.Failed).length
    return {
...
export function formatUpdateTeamUserResult(result: UpdateUsersOnTeamsResult): FormattedUpdateTeamUserTable {
...
export function renderUpdateTeamUserAsciiTable(table: FormattedUpdateTeamUserTable): string {
