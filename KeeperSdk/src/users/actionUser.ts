...
const ACTION_USER_INCLUDES: EnterpriseDataInclude[] = [
...
export async function actionUsers(
  auth: Auth,
  ...
  if (identifiers.length === 0) {
    throw new KeeperSdkError(
      "No users provided.",
      ResultCodes.NO_USERS_TO_ACTION,
    );
  }
  ...
  for (const target of targets) {
    ...
    if (
      input.action === UserAction.Lock ||
      input.action === UserAction.Unlock
    ) {
      if (
        callerEnterpriseUserId !== null &&
        user.enterprise_user_id === callerEnterpriseUserId
      ) {
        logger.warn(
...
async function sendBatchLockUnlock(
  ...
  for (const chunk of chunkArray(enterpriseUserIds, LOCK_UNLOCK_BATCH_SIZE)) {
    ...
    for (const lockResponse of response.response || []) {
      const enterpriseUserId = toEnterpriseUserId(
        lockResponse.enterpriseUserId,
      );
...
function applyLockUserResponse(
  ...
  const status =
    lockResponse.status ?? Enterprise.UserLockStatus.UNKNOWN_LOCK_STATUS;
...
function resolveActionUserTargets(
...
function resolveCallerEnterpriseUserId(
  auth: Auth,
  allUsers: EnterpriseUser[],
): number | null {
...
export function formatUserActionResult(
...
export function renderUserActionAsciiTable(
  table: FormattedUserActionTable,
): string {
