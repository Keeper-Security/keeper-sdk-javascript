...
export const SUPPORTED_USER_COLUMNS: readonly UserColumn[] =
  Object.values(UserColumn);
...
export function formatUsersTable(
  ...
  const headers: string[] = [
    ...
    "User ID",
    "Email",
    ...columns.map((col) => HEADER_BY_COLUMN[col]),
  ];

  const outRows: string[][] = rows.map((row, idx) => {
    const cells: string[] = [
      String(idx + 1),
      String(row.enterprise_user_id),
      row.username,
    ];
...
function rowMatchesPattern(row: ListUserRow, pattern: string): boolean {
  ...
  const addText = (value: string | undefined): void => {
    if (value)
      tokens.push(
        ...value.toLowerCase().split(TOKEN_SEPARATOR_PATTERN).filter(Boolean),
      );
  };
...
function buildUserQueuedTeamMap(
  links: EnterpriseQueuedTeamUserLink[],
): Map<number, Set<string>> {
  const map = new Map<number, Set<string>>();
  for (const link of links) {
    if (!link.team_uid) continue;
...
function buildTeamNameMap(
  teams: Array<EnterpriseTeamRecord | EnterpriseQueuedTeamRecord>,
): Map<string, string> {
  ...
  for (const team of teams) {
    if (team.team_uid)
      map.set(
        team.team_uid,
        (team.name || team.team_uid).trim() || team.team_uid,
      );
  }
  return map;
}
...
function resolveSortedNames<K>(
  keys: Set<K>,
  nameByKey: Map<K, string>,
): string[] {
  const names: string[] = [];
...
function decorateRow(
  ...
  for (const col of columns) {
    switch (col) {
      ...
      case UserColumn.Node:
        row.node =
          user.node_id != null ? (ctx.nodePaths.get(user.node_id) ?? "") : "";
        break;
      case UserColumn.TeamCount:
      ...
      case UserColumn.Teams: {
        ...
        row.teams = teamUids
          ? resolveSortedNames(teamUids, ctx.teamNameByUid)
          : [];
        break;
      }
      case UserColumn.QueuedTeamCount:
        row.queued_team_count =
          ctx.userQueuedTeams.get(user.enterprise_user_id)?.size ?? 0;
        break;
      case UserColumn.QueuedTeams: {
        const queuedUids = ctx.userQueuedTeams.get(user.enterprise_user_id);
        row.queued_teams = queuedUids
          ? resolveQueuedTeamNames(
              queuedUids,
              ctx.teamNameByUid,
              ctx.queuedTeamNameByUid,
            )
          : [];
        break;
      }
      ...
      case UserColumn.Roles: {
        ...
        row.roles = roleIds
          ? resolveSortedNames(roleIds, ctx.roleNameById)
          : [];
        break;
      }
      case UserColumn.Alias: {
