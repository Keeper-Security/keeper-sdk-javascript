...
const UNEXPECTED_TEAM_RESPONSE_MESSAGE =
  "Unexpected API response: no team results returned";
const MISSING_TEAM_RESPONSE_MESSAGE = "Team add response missing for this team";
const UNEXPECTED_REMOVE_RESPONSE_MESSAGE =
  "Unexpected API response: no remove results returned";
const MISSING_REMOVE_RESPONSE_MESSAGE =
  "Team remove response missing for this user";
...
type ResolvedTeam = Pick<
  EnterpriseTeamRecord,
  "team_uid" | "name" | "encrypted_team_key"
>;
...
const toQueuedTeamRecord = (
  team: EnterpriseQueuedTeamRecord,
): EnterpriseTeamRecord => ({
  team_uid: team.team_uid,
  name: team.name,
...
const toPublicKeyBytes = (
  value: Uint8Array | null | undefined,
): Uint8Array | null => (value && value.length > 0 ? value : null);
...
async function fetchUserPublicKeys(
  ...
  try {
    response = await auth.executeRest(
      getPublicKeysMessage({ usernames: emails }),
    );
  } catch (err) {
...
async function encryptTeamKeyForUser(
  ...
  return {
    key: await platform.publicEncryptEC(
      teamKey,
      publicKeys.eccPublicKey as Uint8Array,
    ),
...
async function getDecryptedTeamKey(
  ...
  try {
    ...
    logger.warn(
      `Could not decrypt key for team "${team.name}": ${extractErrorMessage(err)}`,
    );
    cache.set(team.team_uid, null);
    return null;
  }
...
async function loadTeamUserContext(
  ...
  return {
    enterpriseData,
    teams: resolveExistingTeams(
      response.teams || [],
      teamIdentifiers,
      queuedTeams,
    ).map(toResolvedTeam),
    users: resolveExistingUsers(response.users || [], emails),
    membership: buildMembershipSet(
      response.team_users,
...
): Enterprise.TeamUserType | undefined {
...
function pickPublicKeyError(
  publicKeys: UserPublicKeys | undefined,
  username: string,
): string {
  return (
    publicKeys?.message ||
    publicKeys?.errorCode ||
    `No public key for "${username}"`
  );
}

function hasUsablePublicKey(
  publicKeys: UserPublicKeys | undefined,
): publicKeys is UserPublicKeys {
  return (
    !!publicKeys &&
    !publicKeys.errorCode &&
    !!(publicKeys.eccPublicKey || publicKeys.rsaPublicKey)
  );
}
...
function sanitizeResponseMessage(value: string): string {
  return value
    .replace(/[\u0000-\u0008\u000B\u000C\u000E-\u001F\u007F]/g, "")
    .slice(0, MAX_RESPONSE_MESSAGE_LENGTH);
}
...
async function prepareAddBatches(
  ...
  for (const team of teams) {
    ...
    for (const user of users) {
      ...
      if (
        membership.has(membershipKey(user.enterprise_user_id, team.team_uid))
      ) {
        items.push({
          ...base,
...
function markAllBatchUsersFailed(
  batchTeams: PreparedBatchTeam[],
  message: string,
): TeamUserItemResult[] {
...
function mergeTeamResponse(
  ...
  const userMap = new Map(
    prepTeam.prepared.map((p) => [p.user.enterprise_user_id, p.user]),
  );
...
async function sendTeamsEnterpriseUsersAdd(
  ...
  if (teamResponses.length === 0) {
    return markAllBatchUsersFailed(
      batchTeams,
      UNEXPECTED_TEAM_RESPONSE_MESSAGE,
    );
  }
  ...
  for (const teamResp of teamResponses) {
    const teamUid = teamResp.teamUid
      ? webSafe64FromBytes(teamResp.teamUid as Uint8Array)
      : "";
...
async function executeTeamUserCommand(
  auth: Auth,
  command: RestCommand<
    { enterprise_user_id: number; team_uid: string },
    KeeperResponse
  >,
...
async function sendTeamsEnterpriseUsersRemove(
  ...
  const entryMap = new Map(
    entries.map((entry) => [
      membershipKey(entry.user.enterprise_user_id, entry.team.team_uid),
      entry,
    ]),
  );
  const items: TeamUserItemResult[] = [];
  ...
  for (const userResp of removeResponses) {
    ...
    const teamUid = remove?.teamUid
      ? webSafe64FromBytes(remove.teamUid as Uint8Array)
      : "";
...
async function processInvitedQueue(
  auth: Auth,
  invitedQueue: InvitedQueueEntry[],
): Promise<TeamUserItemResult[]> {
  const items: TeamUserItemResult[] = [];
...
export async function addUsersToTeams(
  ...
  const publicKeyMap = await fetchUserPublicKeys(
    auth,
    ctx.users
      .filter((u) => u.status === EnterpriseUserStatus.Active)
      .map((u) => u.username),
  );
  ...
  if (batchTeams.length > 0) {
    items.push(
      ...(await sendTeamsEnterpriseUsersAdd(
        auth,
        batchTeams,
        determineUserType(input.hideSharedFolders),
      )),
    );
  }
...
export async function removeUsersFromTeams(
  ...
  const ctx = await loadTeamUserContext(auth, input.users, input.teams || []);
  const { items, toRemove } = prepareRemoveEntries(
    ctx.teams,
    ctx.users,
    ctx.membership,
  );
...
function finalizeResult(items: TeamUserItemResult[]): TeamUserResult {
  ...
  for (const item of items) {
    if (
      item.status === TeamUserStatus.Added ||
      item.status === TeamUserStatus.Removed
    )
      succeeded++;
...
export function formatTeamUserResult(
  result: TeamUserResult,
): FormattedTeamUserTable {
...
export function renderTeamUserAsciiTable(
  table: FormattedTeamUserTable,
): string {
