...
const USER_UPDATE_TABLE_HEADERS = [
  "#",
  "Status",
  "Email",
  "User ID",
...
export async function updateUsers(
  ...
  const hasProfileChange =
    parentIdentifier !== null || !!input.fullName || !!input.jobTitle;
  ...
  const removeTeamUids = resolveTeamUids(
    input.removeTeam || [],
    response.teams || [],
    response.queued_teams || [],
  );
  const removeRoleIds = resolveRoleIds(
    input.removeRole || [],
    response.roles || [],
    displayNames.roles,
  );
...
async function sendTeamUserRemove(
  auth: Auth,
...
async function sendRoleUserRemove(
  auth: Auth,
  roleId: number,
  enterpriseUserId: number,
): Promise<void> {
...
function resolveTeamUids(
  ...
  for (const id of identifiers) {
    ...
    const uid =
      (uidSet.has(trimmed) ? trimmed : undefined) ??
      byLowerName.get(trimmed.toLowerCase());
    if (!uid) {
      throw new KeeperSdkError(
        `Team "${trimmed}" does not exist.`,
...
function resolveRoleIds(
  ...
  for (const r of roles) {
    ...
    idSet.add(r.role_id);
    const lower = (decryptedRoleNames.get(r.role_id) || "")
      .trim()
      .toLowerCase();
  ...
  for (const id of identifiers) {
    ...
    if (Number.isInteger(numericId))
      roleId = idSet.has(numericId) ? numericId : undefined;
...
export function formatUpdateUserResult(
...
export function renderUpdateUserAsciiTable(
  table: FormattedUpdateUserTable,
): string {
