...
export function formatUserView(
  view: UserView,
  options: FormatUserViewOptions = {},
...
export function userViewTable(table: FormattedUserViewTable): string {
  const labelWidth = table.rows.reduce(
  ...
  const idWidth = table.hasIdColumn
    ? expandedRows.reduce(
        (max, row) => Math.max(max, ...row.idLines.map((l) => l.length)),
        0,
      )
    : 0;
  ...
  for (const row of expandedRows) {
    const lineCount = Math.max(
      ...
      table.hasIdColumn ? row.idLines.length : 0,
      1,
    );
    for (let li = 0; li < lineCount; li += 1) {
      ...
      if (table.hasIdColumn)
        cells.push(padRight(row.idLines[li] ?? "", idWidth));
...
function buildAliases(
  ...
  return links
    .filter(
      (l) =>
        l.enterprise_user_id === user.enterprise_user_id &&
        l.username.toLowerCase() !== primary,
    )
    .map((l) => l.username)
...
function buildUserTeams(
  ...
  const teamByUid = new Map(
    teams.filter((t) => t.team_uid).map((t) => [t.team_uid, t]),
  );
  ...
  return teamUsers
    ...
    .flatMap((link) => {
      ...
      return team
        ? [{ team_uid: team.team_uid, name: team.name || team.team_uid }]
        : [];
    })
    .sort((a, b) =>
...
function buildUserQueuedTeams(
  ...
  for (const qt of queuedTeams) {
    if (qt.team_uid && !teamByUid.has(qt.team_uid))
      teamByUid.set(qt.team_uid, qt.name || qt.team_uid);
  }
...
function buildUserRoles(
  ...
  return roleUsers
    .filter(
      (link) => link.enterprise_user_id === userId && isNumber(link.role_id),
    )
    .map((link) => ({
...
async function fetchShareAdmins(
  auth: Auth,
  username: string,
): Promise<string[]> {
  try {
    ...
    const masked = username.includes("@")
      ...
      : "***";
    logger.warn(
      `get_sharing_admins failed for ${masked}: ${extractErrorMessage(err)}`,
    );
    return [];
  }
}
