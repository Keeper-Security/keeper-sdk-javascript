...
import {
  ...
  type ParseShareExpirationInput,
} from "./nsfTypes";
...
export function isNestedShareRecord(
...
export function resolveKeeperDriveRootParentUid(
  storage: InMemoryStorage,
): string | undefined {
...
export function ensureNestedShareRecord(
  storage: InMemoryStorage,
  recordUid: string,
  identifier?: string,
): void {
  ...
  throw new KeeperSdkError(
    NSF_LEGACY_RECORD_MSG.replace("{0}", ident),
    ResultCodes.NSF_LEGACY_RECORD,
  );
}

export function ensureNestedShareFolder(
  storage: InMemoryStorage,
  folderUid: string,
  identifier?: string,
): void {
  ...
  throw new KeeperSdkError(
    NSF_LEGACY_FOLDER_MSG.replace("{0}", ident),
...
function resolveRecordByTitleSearch(
  storage: InMemoryStorage,
  identifier: string,
): DRecord | undefined {
...
export function resolveNsfRecordIdentifier(
  storage: InMemoryStorage,
  identifier: string,
): string | undefined {
  const trimmed = identifier.trim();
...
export function resolveNsfFolderIdentifier(
  storage: InMemoryStorage,
  identifier: string,
): string | undefined {
  const trimmed = identifier.trim();
...
export function resolveNsfFolderUidOrName(
  storage: InMemoryStorage,
  identifier: string,
): string | undefined {
  const trimmed = identifier.trim();
...
function isVirtualDriveRootParent(
  storage: InMemoryStorage,
  parentUid: string | undefined | null,
): boolean {
...
function folderParentsMatch(
  ...
  if (
    normalizeParentUid(storage, folderParentUid) ===
    normalizeParentUid(storage, searchParentUid)
  ) {
    return true;
  }
  return (
    isRootLevelParent(storage, searchParentUid) &&
    isRootLevelParent(storage, folderParentUid)
  );
}
...
export function findExistingChildFolder(
  ...
  if (byUid) {
    ...
    if (
      normalizeParentUid(storage, byUid.parentUid) ===
      normalizeParentUid(storage, parentUid)
    ) {
      return segment;
    }
    return undefined;
  ...
  for (const folder of getKeeperDriveFolders(storage)) {
    ...
    if (
      normalizeParentUid(storage, folder.parentUid) ===
      normalizeParentUid(storage, parentUid)
    ) {
...
export async function cacheNewNsfFolder(
  ...
  await storage.put({
    ...
    ownerInfo: {
      accountUid: auth.accountUid?.length
        ? webSafe64FromBytes(auth.accountUid)
        : undefined,
      username: auth.username,
    },
...
export function checkFolderDeletePermission(
  ...
  for (const entry of entries) {
    if (!isCurrentUserFolderAccess(storage, entry, username, accountUidStr))
      continue;
    if (
...
export function findNestedShareFoldersForRecord(
  storage: InMemoryStorage,
  recordUid: string,
): string[] {
  return storage
...
function getRecordAccessEntries(
  storage: InMemoryStorage,
  recordUid: string,
): DKdRecordAccess[] {
  return storage
...
function canRecordBeDeleted(
  ...
  for (const entry of entries) {
    if (!isCurrentUserRecordAccess(storage, entry, username, accountUidStr))
      continue;
...
export function checkFolderRemovePermission(
  ...
  if (
    hasFolderPermission(storage, folderUid, username, accountUid, "canRemove")
  )
    return;
  if (canRecordBeDeleted(storage, recordUid, username, accountUid, folderUid))
    return;
  throw new KeeperSdkError(
...
export function formatAccessRoleType(
  role: Folder.AccessRoleType | null | undefined,
): NsfAccessRoleLabel {
...
export function formatAccessType(
  type: Folder.AccessType | null | undefined,
): string {
...
export function getKeeperDriveFolder(
  storage: InMemoryStorage,
  folderUid: string,
): DKdFolder | undefined {
...
export function getKeeperDriveRecord(
  storage: InMemoryStorage,
  recordUid: string,
): DRecord | undefined {
...
export function getFolderAccessEntries(
  storage: InMemoryStorage,
  folderUid: string,
): DKdFolderAccess[] {
  return storage
...
export function findRecordFolderLocation(
  storage: InMemoryStorage,
  recordUid: string,
): string {
  const folderUids = findNestedShareFoldersForRecord(storage, recordUid);
...
export function buildFolderPath(
  ...
  while (
    currentUid &&
    !isRootFolderUid(storage, currentUid) &&
    !seen.has(currentUid)
  ) {
    seen.add(currentUid);
...
export function collectRecordsInFolder(
  storage: InMemoryStorage,
  folderUid: string,
): DRecord[] {
...
export function resolveAccessUsername(
  ...
  if (
    folder?.ownerInfo?.accountUid === accessTypeUid &&
    folder.ownerInfo.username
  ) {
...
export async function loadShareUserMap(
  auth: Auth,
  storage: InMemoryStorage,
...
export function isFolderOwnerAccessor(
  ...
  if (
    folder.ownerInfo?.accountUid &&
    folder.ownerInfo.accountUid === entry.accessTypeUid
  )
    return true;
  return entry.accessType === Folder.AccessType.AT_OWNER;
}

export function recordAccessDisplayRole(
...
> {
  try {
    const [response, shareUsers] = await Promise.all([
      auth.executeRest(
        getRecordAccessMessage({ recordUids: [normal64Bytes(recordUid)] }),
      ),
    ...
    return (response.recordAccesses ?? [])
      ...
      .map((entry) => {
        ...
        if (!data?.accessTypeUid?.length || data.accessType == null)
          return undefined;
...
export function folderAccessDisplayRole(
  entry: DKdFolderAccess,
): NsfAccessRoleLabel {
  if (entry.accessType === Folder.AccessType.AT_OWNER)
    return NsfAccessRoleLabel.Owner;
...
export async function patchNsfFolderInheritPermissions(
  storage: InMemoryStorage,
  folderUid: string,
  inheritPermissions: boolean,
): Promise<void> {
...
export function checkRecordChangeOwnershipPermission(
  ...
  if (canChangeRecordOwnership(storage, recordUid, username, accountUid))
    return;
  throw new KeeperSdkError(
...
export function findFolderAccessEntry(
  ...
  return getFolderAccessEntries(storage, folderUid).find(
    (entry) =>
      entry.accessTypeUid === accessTypeUid && entry.accessType === accessType,
  );
}

function toFolderAccessEntry(
  folderUid: string,
  accessor: Folder.IFolderAccessData,
): DKdFolderAccess {
  return {
...
export async function fetchLiveFolderAccessEntries(
  ...
  try {
    ...
    const result = response.folderAccessResults?.find(
      (entry) =>
        entry.folderUid?.length &&
        webSafe64FromBytes(entry.folderUid) === folderUid,
    );
...
export async function findFolderAccessEntryOrLive(
  ...
  const fromStorage = findFolderAccessEntry(
    storage,
    folderUid,
    accessTypeUid,
    accessType,
  ...
  try {
    ...
    return liveEntries.find(
      (entry) =>
        entry.accessTypeUid === accessTypeUid &&
...
export function normalizeNsfRecordPermissionRole(
  role?: string,
): string | undefined {
  if (!role?.trim()) return undefined;
  const normalized = role.trim().toLowerCase().replace(/\s+/g, "-");
  if ((NSF_RECORD_PERMISSION_ROLES as readonly string[]).includes(normalized))
    return normalized;
...
function mapLiveRecordAccessEntry(
  ...
  const data = entry.data;
  if (
    !data?.recordUid?.length ||
    !data.accessTypeUid?.length ||
...
export async function fetchLiveRecordAccessesV3(
  ...
  for (
    let index = 0;
    index < recordUids.length;
    index += NSF_SHARE_BATCH_SIZE
  ) {
...
export function findUserRecordShareEntry(
  ...
  for (const entry of getRecordAccessEntries(storage, recordUid)) {
    ...
    const accessorName = resolveAccessUsername(
      storage,
      entry.accessTypeUid,
      undefined,
      shareUsers,
...
export function parseShareExpiration(
  input: ParseShareExpirationInput,
): number | undefined {
  const {
    expireAt,
    expireIn,
...
export function isShareExpirationNoop(
  ...
  if (requestedExpiration === -1) {
    return (
      ...
      existingExpiration === -1 ||
      existingExpiration === 0
    );
  }
  if (existingExpiration == null || existingExpiration === 0) return false;
  return (
    Math.abs(existingExpiration - requestedExpiration) <=
    NSF_TLA_EXPIRATION_TOLERANCE_MS
  );
}
