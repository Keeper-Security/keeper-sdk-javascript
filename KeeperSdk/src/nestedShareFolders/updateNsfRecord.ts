...
function toUpdateItems(input: UpdateNsfRecordInput | UpdateNsfRecordsInput): UpdateNsfRecordItemInput[] {
...
async function buildRecordUpdatePayload(
    ...
    const mergedRecordData = mergeNsfRecordData(loadStoredRecordData(storage, recordUid), changes)
    return {
        recordUid,
        storedRecord,
        mergedRecordData,
...
export async function updateNestedShareRecords(
    ...
    for (const recordType of recordTypes) {
        await validateNsfRecordType(auth, recordType, ResultCodes.NSF_UPDATE_FAILED)
    }
    ...
    try {
        ...
        for (const item of items) {
            ...
            payloads.push(await buildRecordUpdatePayload(storage, auth, recordUid, changes))
        }
