...
const OPERATION_MAP: Record<
  NsfRemoveFolderOperation,
  FolderProto.v3.remove.FolderOperationType
> = {
  [NsfRemoveFolderOperation.FolderTrash]:
    FolderOperationType.FOLDER_MOVE_TO_FOLDER_TRASH,
  [NsfRemoveFolderOperation.DeletePermanent]:
    FolderOperationType.FOLDER_DELETE_PERMANENT,
};
...
function buildRemovals(
  ...
  if (folderIdentifiers.length === 0) {
    throw new KeeperSdkError(
      "Enter the name or UID of at least one folder.",
...
function toRemovalInput(
  spec: RemovalSpec,
): FolderProto.v3.remove.IFolderRemoval {
  return {
...
function mapPreviewItem(
  spec: RemovalSpec,
  item: FolderProto.v3.remove.IRemoveResult,
): NsfRemoveFolderPreviewItem {
  return {
    folderUid: item.itemUid?.length
      ? webSafe64FromBytes(item.itemUid)
      : spec.folderUid,
    name: spec.name,
    status:
      item.status == null
        ? "REMOVE_STATUS_UNKNOWN"
        : (RemoveStatus[item.status] ?? String(item.status)),
    impact: item.impact
      ? {
...
function mapPreview(
  ...
  return (response.results ?? []).map((item, index) => {
    ...
    if (!spec) {
      throw new KeeperSdkError(
        "Folder removal preview mismatch.",
        ResultCodes.NSF_REMOVE_FAILED,
      );
    }
...
function hasPreviewErrors(preview: NsfRemoveFolderPreviewItem[]): boolean {
  return preview.some(
    (item) => item.error != null || item.status !== REMOVE_SUCCESS_STATUS,
  );
}
...
export async function removeNestedShareFolders(
  ...
  try {
    const previewResponse = await executeRemove(
      auth,
      removals,
      RemoveAction.REMOVE_ACTION_PREVIEW,
    );
    const preview = mapPreview(removals, previewResponse);

    if (hasPreviewErrors(preview)) {
      throw new KeeperSdkError(
        formatRemoveNsfFolderPreview(preview, operation, input.quiet) ||
          "Folder removal preview failed.",
        ResultCodes.NSF_REMOVE_FAILED,
      );
    }
    ...
    await executeRemove(
      auth,
      removals,
      RemoveAction.REMOVE_ACTION_CONFIRM,
      previewResponse.confirmationToken,
    );
    const actionLabel =
      operation === NsfRemoveFolderOperation.DeletePermanent
        ? "Permanently deleted"
