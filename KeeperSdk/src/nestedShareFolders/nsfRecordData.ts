...
import {
  NSF_KNOWN_FIELD_TYPES,
  NSF_LEGACY_RECORD_TYPES,
  NSF_STRUCTURED_SUBKEYS,
} from "./nsfConstants";
...
function splitFieldTokens(
  ...
  if (inSingleQuote) {
    throw new KeeperSdkError(
      "Unclosed single quote in field input.",
      ResultCodes.NSF_ADD_FAILED,
    );
  }
  if (inDoubleQuote) {
    throw new KeeperSdkError(
      "Unclosed double quote in field input.",
      ResultCodes.NSF_ADD_FAILED,
    );
  }
  if (braceDepth > 0) {
    throw new KeeperSdkError(
      'Unclosed "{" in field input.',
      ResultCodes.NSF_ADD_FAILED,
    );
  }
...
function parseFieldToken(
  raw: string,
  position: number,
): FieldAssignment | "file" | undefined {
...
function wrapPlainStructuredValue(
  fieldType: string,
  value: string,
): Record<string, unknown> {
...
export function buildNsfRecordData(
  input: BuildNsfRecordDataInput,
): Record<string, unknown> {
...
export function mergeNsfRecordData(
  ...
  if (input.fieldEntries?.length) {
    data.fields = mergeFieldEntries(
      data.fields as RecordFieldEntry[],
      input.fieldEntries,
    );
  }
  if (input.customEntries?.length) {
    data.custom = mergeFieldEntries(
      data.custom as RecordFieldEntry[],
      input.customEntries,
    );
  }
