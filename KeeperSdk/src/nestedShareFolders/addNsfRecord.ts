...
async function requireFolderKey(
  storage: InMemoryStorage,
  folderUid: string,
): Promise<Uint8Array> {
...
async function buildRecordAddPayload(
  ...
  if (folderUid) {
    ...
    recordAdd.recordKeyEncryptedBy =
      Folder.FolderKeyEncryptionType.ENCRYPTED_BY_PARENT_KEY;
    recordAdd.recordKeyType = Folder.EncryptedKeyType.encrypted_by_data_key_gcm;
  } else {
    recordAdd.recordKey = await platform.aesGcmEncrypt(
      recordKey,
      requireAuthDataKey(auth),
    );
    recordAdd.recordKeyType = Folder.EncryptedKeyType.encrypted_by_data_key_gcm;
  }
...
function buildRecordDataFromInput(
...
export async function addNestedShareRecord(
  ...
  if (!added[0]) {
    throw new KeeperSdkError(
      "Failed to add nested share record.",
      ResultCodes.NSF_ADD_FAILED,
    );
  }
  return added[0];
