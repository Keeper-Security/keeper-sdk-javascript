...
function isShortcutFolder(storage: InMemoryStorage, folderUid: string): boolean {
    ...
    return isRootFolderUid(storage, normalized) || isNestedShareFolder(storage, normalized)
}
...
function resolveKeepFolderUid(
    ...
    if (folderArg?.trim()) {
        ...
        return normalizeParentUid(storage, folderUid)
    }

    if (defaultFolderUid && isNestedShareFolder(storage, defaultFolderUid)) {
        return normalizeParentUid(storage, defaultFolderUid)
    }
