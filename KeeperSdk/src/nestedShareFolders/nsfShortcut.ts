...
import { ListNsfFormat, type ListNsfFormatInput } from "./nsfTypes";
import {
  KeeperDriveKind,
...
function isShortcutFolder(
  storage: InMemoryStorage,
  folderUid: string,
): boolean {
  return (
    isRootFolderUid(storage, folderUid) ||
    isNestedShareFolder(storage, folderUid)
  );
}

function formatFolderLabel(
  storage: InMemoryStorage,
  folderUid: string,
): string {
  ...
  const name = getFolderDisplayName(storage, folderUid);
  return `${name} (${folderUid})`;
}

export function getNsfRecordShortcuts(
  storage: InMemoryStorage,
  ...
  return new Map(
    [...records.entries()].filter(([, folders]) => folders.size > 1),
  );
}
...
function resolveShortcutRecordUids(
  ...
  if (folderUid) {
    const matches = [...shortcuts.entries()]
      .filter(([, folders]) => folders.has(folderUid))
...
function collectShortcutRows(
  ...
  return [...recordUids]
    .sort((a, b) =>
      recordTitle(storage, a).localeCompare(recordTitle(storage, b)),
    )
    .map((recordUid) => ({
      recordUid,
      ...
      folders: [...(shortcuts.get(recordUid) ?? [])]
        .sort((a, b) =>
          formatFolderLabel(storage, a).localeCompare(
            formatFolderLabel(storage, b),
          ),
        )
...
export function formatNsfShortcutTable(rows: NsfShortcutRow[]): string {
  ...
  const columnWidths = headers.map((header, index) =>
    Math.max(
      header.length,
      ...tableRows.map((cells) => (cells[index] || "").length),
    ),
  );
...
function parseFolderRecordUpdateResult(
  ...
  const result = response.folderRecordUpdateResult?.find(
    (entry) =>
      !!entry.recordUid?.length &&
      webSafe64FromBytes(entry.recordUid) === recordUid,
  );
...
export function formatKeepNsfShortcutPlan(
...
export async function keepNsfShortcut(
  ...
  if (!recordArg) {
    throw new KeeperSdkError(
      "Record UID or title is required.",
      SHORTCUT_ERROR,
    );
  }

  const shortcuts = getNsfRecordShortcuts(storage);
  const recordMatches = resolveShortcutRecordUids(
    storage,
    recordArg,
    shortcuts,
  );
  ...
  const keepFolderUid = resolveKeepFolderUid(
    storage,
    input.folder,
    defaultFolderUid,
  );
  ...
  const removeFolderUids = [...folderSet].filter(
    (folderUid) => folderUid !== keepFolderUid,
  );
  const plan: KeepNsfShortcutPlanItem = {
    ...
    removeFolderUids,
    removeFolderLabels: removeFolderUids.map((folderUid) =>
      formatFolderLabel(storage, folderUid),
    ),
  };
