...
function mapUserShareKeysEntry(
  email: string,
  entry: NonNullable<
    Authentication.IGetPublicKeysResponse["keyResponses"]
  >[number],
): UserShareKeys | null {
  ...
  return {
    ...
    rsaPublicKey: entry.publicKey?.length
      ? (entry.publicKey as Uint8Array)
      : null,
    eccPublicKey: entry.publicEccKey?.length
...
async function fetchUserShareKeys(
  auth: Auth,
  email: string,
): Promise<UserShareKeys | null> {
  const response = await auth.executeRest(
    getPublicKeysMessage({ usernames: [email] }),
  );
...
export async function loadUserShareKeys(
  auth: Auth,
  email: string,
...
export async function loadNsfUserShareKeysMap(
  ...
  const response = await auth.executeRest(
    getPublicKeysMessage({ usernames: deduped }),
  );
...
export async function preloadNsfUserShareKeys(
  ...
  for (const emailKey of uniqueEmails) {
    ...
    if (
      !userKeys ||
      (!userKeys.rsaPublicKey?.length && !userKeys.eccPublicKey?.length)
    ) {
...
export function parseRecordSharingStatus(
  ...
  const sharingStatus =
    status.status ?? record.v3.sharing.SharingStatus.SUCCESS;
  const statusName =
    record.v3.sharing.SharingStatus[sharingStatus] ?? String(sharingStatus);
  const success =
...
export async function buildNsfSharePermissionFromKeys(
  ...
  const { encryptedKey, useEccKey } = await encryptKeyForRecipient(
    recordKey,
    userKeys,
  );
  ...
  if (expirationTimestamp != null) {
    ...
    if (expirationTimestamp > 0) {
      rules.tlaProperties.timerNotificationType =
        common.tla.TimerNotificationType.NOTIFY_OWNER;
    }
  }
  return {
