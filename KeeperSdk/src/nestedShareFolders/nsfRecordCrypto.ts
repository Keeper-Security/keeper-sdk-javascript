...
async function decryptWithFolderKeys(
  ...
  for (const folderUid of findNestedShareFoldersForRecord(storage, recordUid)) {
    ...
    try {
      ...
      try {
        const recordKey = await platform.aesCbcDecrypt(
          encryptedKey,
          folderKey,
          true,
        );
  ...
  logger.debug(
    `NSF record key decrypt failed for ${recordUid}: no folder key succeeded`,
  );
  return undefined;
}
...
export async function decryptRecordTitleAndType(
  ...
  const uid = recordData.recordUid?.length
    ? webSafe64FromBytes(recordData.recordUid)
    : recordUid;
  ...
  try {
    ...
    const parsed = JSON.parse(
      platform.bytesToString(decrypted).replace(/\s+$/, ""),
    ) as {
      title?: string;
      type?: string;
    };
    ...
    logger.debug(
      `NSF record title/type decrypt failed for ${uid}: ${extractErrorMessage(err)}`,
    );
    return { title: UNKNOWN_RECORD_LABEL, type: UNKNOWN_RECORD_LABEL };
  }
}
