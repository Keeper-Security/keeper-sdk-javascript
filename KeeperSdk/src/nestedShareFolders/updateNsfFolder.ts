...
function assertHasUpdateFields(input: {
  ...
  if (
    ...
    input.inheritPermissions === undefined
  ) {
    throw new KeeperSdkError(
...
async function buildUpdateFolderData(
  ...
  const resolvedParentUid = resolveKeeperDriveParentUid(
    storage,
    stored?.parentUid,
  );
...
async function prepareFolderUpdate(
  ...
  if (!folderArg) {
    throw new KeeperSdkError(
      "Enter the path or UID of existing folder.",
      ResultCodes.NSF_UPDATE_FAILED,
    );
  }
  ...
  const folderData = await buildUpdateFolderData(
    storage,
    folderUid,
    metadata,
    inheritPermissions,
...
async function persistFolderUpdate(
  storage: InMemoryStorage,
  prepared: PreparedFolderUpdate,
): Promise<void> {
  // Always refresh local folder data to match the encrypted payload we sent.
  await patchNsfFolderMetadata(
    storage,
    prepared.folderUid,
    prepared.metadata,
  );
  if (prepared.inheritPermissions !== undefined) {
    await patchNsfFolderInheritPermissions(
      storage,
      prepared.folderUid,
...
export async function updateNestedShareFolder(
  ...
  try {
    ...
    const response = await auth.executeRest(
      folderUpdateMessage({ folderData: [prepared.folderData] }),
    );
    parseFolderModifyStatus(
      response.folderUpdateResults?.[0],
      ResultCodes.NSF_UPDATE_FAILED,
    );
...
export async function updateNestedShareFolders(
  ...
  if (!updates?.length) {
    throw new KeeperSdkError(
      "At least one folder update is required.",
      ResultCodes.NSF_UPDATE_FAILED,
    );
  }
  ...
  try {
    ...
    for (let i = 0; i < preparedAll.length; i += NSF_MAX_FOLDER_UPDATES) {
      ...
      for (let j = 0; j < batch.length; j++) {
        ...
        const status =
          modifyResult?.status ?? Folder.FolderModifyStatus.SUCCESS;
        const success = status === Folder.FolderModifyStatus.SUCCESS;
        const statusName =
          Folder.FolderModifyStatus[status] ?? String(status);
        const message =
