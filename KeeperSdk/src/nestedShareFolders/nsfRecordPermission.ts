...
function normalizeAction(
  ...
  if (
    value === NsfRecordPermissionAction.Grant ||
    value === NsfRecordPermissionAction.Revoke
  ) {
    return value;
  }
...
async function getRecordAccessesV3(
  ...
  if (recordUids.length === 0) {
    throw new KeeperSdkError(
      "At least one record UID required.",
      ResultCodes.NSF_RECORD_PERMISSION_FAILED,
    );
  }
...
async function buildSharePermission(
  ...
  if (!userKeys) {
    userKeys = await loadUserShareKeysOrInvite(
      auth,
      item.email,
      ResultCodes.NSF_RECORD_PERMISSION_FAILED,
    );
...
async function runShareBatch<T extends NsfSharePermissionItem>(
  ...
  applyToRequest: (
    request: record.v3.sharing.IRequest,
    permission: record.v3.sharing.IPermissions,
  ) => void,
  statusField:
    | "updatedSharingStatus"
    | "createdSharingStatus"
  ...
  for (let index = 0; index < items.length; index += NSF_SHARE_BATCH_SIZE) {
    ...
    const userKeysByEmail = await preloadUserShareKeys(
      auth,
      chunk,
      inviteMissingKeys,
    );
...
async function batchUpdateRecordSharesV3(
  ...
  return runShareBatch(
    auth,
    updates,
    (item, userKeysByEmail) =>
      buildSharePermission(storage, auth, item, userKeysByEmail),
    (request, permission) => {
...
async function batchUnshareRecordsV3(
  ...
  return runShareBatch(
    ...
    (request, permission) => {
      request.revokeSharingPermissions = [
        ...(request.revokeSharingPermissions ?? []),
        permission,
      ];
    },
  ...
  for (const access of accesses) {
    ...
    if (isInherited) {
      skipped.push({
        ...
        curRole,
        reason:
          "Inherited from a shared folder — revoke at the parent shared folder",
      });
      continue;
    }
...
function formatPermissionPlanTable(
  ...
  const formatRow = (cells: string[]) =>
    cells.map((cell, columnIndex) => pad(cell, columnIndex)).join("  ");
  const rule = widths
    .map((width, columnIndex) => pad("-".repeat(width), columnIndex))
    .join("  ");
...
export function formatNsfRecordPermissionPlan(
  ...
  if (plan.grants.length > 0) {
    ...
    lines.push(
      "WARNING: Review the planned changes carefully before confirming.",
    );
    lines.push("");
  }
...
export async function updateNestedShareRecordPermissions(
  ...
  if (action === NsfRecordPermissionAction.Grant) {
    if (!normalizedRole) {
      throw new KeeperSdkError(
        "Role is required for grant action.",
        ResultCodes.NSF_RECORD_PERMISSION_FAILED,
      );
    }
    if (
      !(NSF_RECORD_PERMISSION_ROLES as readonly string[]).includes(
        normalizedRole,
      )
    ) {
      throw new KeeperSdkError(
  ...
  const { folderUid, displayName } = resolveFolderForPermission(
    storage,
    input.folder,
  );
  ...
  try {
    const accessesResult = await getRecordAccessesV3(auth, storage, [
      ...recordUids,
    ]);
    ...
    if (
      buckets.updates.length === 0 &&
      buckets.creates.length === 0 &&
      buckets.revokes.length === 0
    ) {
      return {
        confirmed: false,
        dryRun,
    ...
    if (buckets.updates.length > 0) {
      grantOutcomes.push(
        ...(await batchUpdateRecordSharesV3(storage, auth, buckets.updates)),
      );
    }
    if (buckets.creates.length > 0) {
      grantOutcomes.push(
        ...(await batchCreateRecordSharesV3(storage, auth, buckets.creates)),
      );
    }

    const revokeOutcomes =
      buckets.revokes.length > 0
        ? await batchUnshareRecordsV3(auth, buckets.revokes)
        : [];
...
export function formatNsfRecordPermissionFailures(
  ...
  for (const failure of failures) {
    lines.push(
      `  ${failure.recordUid}  ${failure.email}  ${failure.code}  ${failure.message}`,
    );
  }
  return lines.join("\n");
}
