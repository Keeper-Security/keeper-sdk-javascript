...
function normalizeFolderAction(
  action: NsfFolderShareActionInput = NsfFolderShareAction.Grant,
): NsfFolderShareAction {
  const value = action as NsfFolderShareAction;
  if (
    value === NsfFolderShareAction.Grant ||
    value === NsfFolderShareAction.Remove
  )
    return value;
  throw new KeeperSdkError(
    `Invalid action '${action}'. Use: grant, remove.`,
    SHARE_ERROR,
  );
}

function normalizeRecordAction(
  action: NsfRecordShareActionInput = NsfRecordShareAction.Grant,
): NsfRecordShareAction {
  ...
  throw new KeeperSdkError(
    `Invalid action '${action}'. Use: grant, revoke, owner.`,
    SHARE_ERROR,
  );
}
...
async function buildEncryptedFolderKeyForUser(
  ...
  throw new KeeperSdkError(
    `No usable public key available for ${email}`,
    SHARE_ERROR,
  );
}
...
async function buildFolderAccessGrantData(
  ...
  const accessType = target.isTeam
    ? Folder.AccessType.AT_TEAM
    : Folder.AccessType.AT_USER;
  ...
  if (target.isTeam) {
    ...
    const teamKeys = await fetchNsfTeamPublicKeys(
      auth,
      target.recipient,
      storage,
    );
    ...
    const userKeys = await loadUserShareKeysOrInvite(
      auth,
      target.recipient,
      SHARE_ERROR,
    );
    accessTypeUid = userKeys.accountUid;
    encryptedFolderKey = await buildEncryptedFolderKeyForUser(
      auth,
      folderKey,
      target.recipient,
    );
...
async function resolveFolderShareTargets(
  ...
  for (const raw of recipients) {
    ...
    if (trimmed === "@existing" || trimmed === "@current") {
      for (const entry of collectExistingFolderShareTargets(
        storage,
        folderUid,
        currentUsername,
      )) {
        ...
        targets.push({
          ...
          accountUid: entry.accountUid
            ? normal64Bytes(entry.accountUid)
            : undefined,
        });
      }
      continue;
...
async function grantFolderAccess(
  ...
  const accessType = target.isTeam
    ? Folder.AccessType.AT_TEAM
    : Folder.AccessType.AT_USER;
  const accessTypeUid = target.accountUid
    ? webSafe64FromBytes(target.accountUid)
    : target.recipient;
  ...
  if (
    existing &&
    existing.accessRoleType === accessRoleType &&
  ...
  const response = await auth.executeRest(
    folderAccessUpdateMessage({
      [existing != null ? "folderAccessUpdates" : "folderAccessAdds"]: [
        request,
      ],
    }),
  );
  ...
  return {
    ...
    actionTaken:
      existing != null
        ? NsfFolderShareActionTaken.Updated
        : NsfFolderShareActionTaken.Granted,
...
async function removeFolderAccess(
  ...
  const accountUid =
    ...
    (target.isTeam
      ...
      : (await loadUserShareKeysOrInvite(auth, target.recipient, SHARE_ERROR))
          .accountUid);

  const response = await auth.executeRest(
    folderAccessUpdateMessage({
      folderAccessRemoves: [
        buildFolderAccessRemoveData(folderUid, accountUid, accessType),
      ],
    }),
  );
...
function resolveRecordUids(
  ...
  if (folderUid) {
    ...
    const recordUids = collectNsfRecordUidsInFolder(
      storage,
      folderUid,
      recursive,
    );
    if (recordUids.size === 0) {
      throw new KeeperSdkError(
        "No records found in the specified folder.",
        ResultCodes.NSF_NOT_FOUND,
      );
    }
    return [...recordUids];
  ...
  throw new KeeperSdkError(
    `Record or folder '${trimmed}' not found`,
    ResultCodes.NSF_NOT_FOUND,
  );
}
...
async function transferRecordOwnership(
  ...
  const result = await transferNestedShareRecordOwnership(
    storage,
    auth,
    recordUid,
    email,
...
async function resolveUserRecordShareForRevoke(
  ...
  const storageEntry = findUserRecordShareEntry(
    storage,
    recordUid,
    email,
    accountUidStr,
...
async function revokeRecordShare(
  ...
  const shareState = await resolveUserRecordShareForRevoke(
    auth,
    storage,
    recordUid,
    email,
  ...
  if (shareState.inherited) {
    return {
      ...
      message:
        "Access is inherited from a shared folder — revoke at the parent folder",
    };
  }
...
async function grantRecordShare(
  ...
  if (
    existing &&
    isShareUpdateNoop(existing, accessRoleType, expirationTimestamp)
  ) {
    return {
      recordUid,
      email,
  ...
  if (existing && expirationTimestamp != null && expirationTimestamp > 0) {
    const revokeResult = await revokeRecordShare(
      storage,
      auth,
      recordUid,
      email,
  ...
  const response = await auth.executeRest(
    recordsShareV3Message({ [field]: [permission] }),
  );
  const statusField = existing
  ...
  return {
    ...
    actionTaken: existing
      ? NsfRecordShareActionTaken.Update
      : NsfRecordShareActionTaken.Grant,
    message: parsed.message,
  };
}
...
export function formatNsfRecordSharePlan(
...
export function formatNsfRecordShareResults(
...
export function formatNsfFolderShareResults(
...
export async function shareNestedShareRecord(
  ...
  const recordUids = resolveRecordUids(
    storage,
    input.record,
  ...
  try {
    for (const email of emails) {
      for (const recordUid of recordUids) {
        if (action === NsfRecordShareAction.Owner) {
          results.push(
            await transferRecordOwnership(storage, auth, recordUid, email),
          );
        } else if (action === NsfRecordShareAction.Revoke) {
          results.push(
            await revokeRecordShare(storage, auth, recordUid, email),
          );
        } else {
          results.push(
