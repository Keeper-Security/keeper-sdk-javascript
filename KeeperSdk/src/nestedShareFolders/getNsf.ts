...
function recordDetailsMessage(
  recordUid: string,
  include: Records.RecordDetailsInclude,
) {
...
function buildFolderAccessRow(
  ...
  const username = resolveAccessUsername(
    storage,
    entry.accessTypeUid,
    folder,
    shareUsers,
  ...
  for (const entry of entries) {
    if (isFolderUserPermission(entry)) {
      userPermissions.push(
        buildFolderAccessRow(storage, folder, entry, shareUsers),
      );
    }
...
function prependOwnerRow(
  rows: NsfFolderAccessRow[],
      ...
      value:
        masked && stringValues.some((value) => value.length > 0)
          ? stringValues.map((value) =>
              value.length > 0 ? NSF_MASKED_VALUE : value,
            )
          : stringValues,
    };
  });
...
async function fetchRecordShareAdmins(
  auth: Auth,
  recordUid: string,
): Promise<string[]> {
  try {
...
async function fetchRecordDataFallback(
  auth: Auth,
  recordUid: string,
): Promise<DRecord["data"] | undefined> {
  try {
    const response = await auth.executeRest(
      recordDetailsMessage(recordUid, Records.RecordDetailsInclude.DATA_ONLY),
    );
    return response.recordDataWithAccessInfo?.[0]?.recordData as
      | DRecord["data"]
      | undefined;
  } catch {
    return undefined;
...
async function buildFolderView(
  ...
  if (!folder) {
    throw new KeeperSdkError(
      `Nested share folder not found: ${folderUid}`,
...
async function buildRecordView(
  ...
  if (!record) {
    throw new KeeperSdkError(
      `Nested share record not found: ${recordUid}`,
      ResultCodes.NSF_NOT_FOUND,
    );
  }
  ...
  const notes =
    typeof record.data?.notes === "string" && record.data.notes.trim()
      ? record.data.notes.trim()
      : undefined;
...
export function resolveNsfFolder(
  storage: InMemoryStorage,
  identifier: string,
): string | undefined {
...
export function resolveNsfRecord(
  storage: InMemoryStorage,
  identifier: string,
): string | undefined {
...
export async function getNestedShareFolder(
  ...
  if (recordUid) {
    const view = await buildRecordView(
      auth,
      storage,
      recordUid,
      options.unmask ?? false,
    );
...
export function formatNsfFolderDetail(
  view: NsfFolderView,
  verbose = false,
): string {
  const lines = [
...
export function formatNsfRecordDetail(
  view: NsfRecordView,
  verbose = false,
): string {
  const lines = [
