...
const OPERATION_MAP: Record<
  NsfRemoveOperation,
  FolderProto.v3.remove.RecordOperationType
> = {
...
function normalizeOperation(
  operation: NsfRemoveOperationInput = NsfRemoveOperation.OwnerTrash,
): NsfRemoveOperation {
...
function buildRemovals(
  ...
  const folderUid = folderIdentifier
    ? resolveNsfFolderIdentifier(storage, folderIdentifier)
    : undefined;
  if (folderIdentifier && !folderUid) {
    throw new KeeperSdkError(
      `Folder '${folderIdentifier}' not found`,
      ResultCodes.NSF_NOT_FOUND,
    );
  }
...
export function collectRemoveNsfWarnings(
  preview: NsfRemovePreviewItem[],
): string[] {
...
export function formatRemoveNsfPreview(
  preview: NsfRemovePreviewItem[],
): string {
  const lines: string[] = [];
  for (const item of preview) {
