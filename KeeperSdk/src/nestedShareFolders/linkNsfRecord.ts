...
function resolveRecordKeyType(
  ...
  const metadata = storage.getByUid<DRecordMetadata>(
...
async function buildRecordMetadata(
  ...
  const encryptedRecordKey = await platform.wrapKey(
    recordUid,
    folderUid,
    encryptionType,
    storage,
...
function parseFolderRecordUpdateResponse(
  ...
  const statusName =
    Folder.FolderModifyStatus[
      result.status ?? Folder.FolderModifyStatus.SUCCESS
    ] ?? "UNKNOWN";
  const success = result.status === Folder.FolderModifyStatus.SUCCESS;
  return {
    success,
    recordUid: result.recordUid?.length
      ? webSafe64FromBytes(result.recordUid)
      : recordUid,
    folderUid: response.folderUid?.length
      ? webSafe64FromBytes(response.folderUid)
      : folderUid,
    status: statusName,
    message:
      result.message ||
...
export async function linkNestedShareRecord(
  ...
  if (!recordUid) {
    throw new KeeperSdkError(
      `Record '${recordIdentifier}' not found`,
  ...
  try {
    const recordMetadata = await buildRecordMetadata(
      storage,
      folderUid,
      recordUid,
    );
    ...
    const parsed = parseFolderRecordUpdateResponse(
      response,
      folderUid,
      recordUid,
    );
