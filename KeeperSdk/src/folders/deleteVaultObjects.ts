...
function folderKindOfUid(
  ...
  if (storage.getByUid<DSharedFolder>("shared_folder", uid))
    return "shared_folder";
  if (storage.getByUid<DSharedFolderFolder>("shared_folder_folder", uid))
...
export async function deleteVaultObjects(
  ...
  if (!token) {
    return {
      success: false,
      message:
        preResp.message || preResp.result_code || "pre_delete failed: no token",
    };
  }
