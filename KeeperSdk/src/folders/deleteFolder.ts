...
function folderKindOfUid(storage: InMemoryStorage, uid: string): FolderKind {
  ...
  if (storage.getByUid<DSharedFolderFolder>(FolderKind.SharedFolderFolder, uid))
...
export async function deleteFolder(
  ...
  const targetUids = objects
    .map((deleteObject) => deleteObject.object_uid)
    .join(", ");
  ...
  try {
    await auth.executeRestCommand(
      recordDeleteCommand({ pre_delete_token: deleteToken }),
    );
  } catch (err) {
    return {
      success: false,
...
async function dedupeNestedFolderDeletes(
  storage: InMemoryStorage,
...
export async function rmdir(
  ...
  for (const pattern of patterns) {
    ...
    const resolved = await resolveRmdirPatternsToFolderUids(
      storage,
      session,
      trimmedPattern,
    );
