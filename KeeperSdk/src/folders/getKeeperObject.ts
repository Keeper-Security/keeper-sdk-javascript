    ...
    return {
      legacy: {
        ...
        custom: d.custom?.map((c) => ({
          ...c,
          value:
            c.type && KEEPER_SENSITIVE_FIELD_TYPES.has(c.type) && !unmask
              ? "********"
              : (c.value ?? ""),
        })),
      },
      permissions: meta
        ? { owner: meta.owner, canShare: meta.canShare, canEdit: meta.canEdit }
        : undefined,
    };
  }
...
async function buildRecordResult(
  ...
  if (format === "json") {
    base.json = {
      ...
      revision: record.revision,
      shared: record.shared,
      detail: JSON.parse(JSON.stringify(base.detail)) as Record<
        string,
        unknown
      >,
    };
...
export async function getKeeperObject(
  ...
  if (!target) {
    throw new KeeperSdkError(
      `"${trimmed}" not found as any Keeper object`,
      "keeper_object_not_found",
    );
  }
