...
function decideNewFolderType(
  ...
  if (isSharedFolder) {
    if (
      parent.kind !== ParentFolderKind.UserFolder &&
      parent.kind !== ParentFolderKind.VirtualRoot
    ) {
      throw new KeeperSdkError(
...
export async function addFolder(
  ...
  const sharedScope =
    folderType === FolderKind.SharedFolderFolder ? parent.sharedScopeUid : null;
  ...
  if (folderType === FolderKind.SharedFolder) {
    request.name = await encryptForStorage(
      platform.stringToBytes(name),
      folderKey,
    );
    request.manage_users = isBoolean(input.manageUsers)
      ? input.manageUsers
      : false;
  ...
  try {
    ...
    const succeeded =
      response.result === FolderResultStatus.Success ||
      response.result_code === FolderResultStatus.Success;
    if (!succeeded) {
      const reason =
        response.message ||
...
export async function mkdir(
  ...
  let manageUsers = isBoolean(options.manageUsers)
    ? options.manageUsers
    : false;
  let manageRecords = isBoolean(options.manageRecords)
    ? options.manageRecords
    : false;
  ...
  for (let segmentIndex = 0; segmentIndex < segments.length; segmentIndex++) {
    ...
    const existingChildUid = await findChildFolderUidByName(
      storage,
      currentParent,
      segment,
    );
    ...
    lastResult = await addFolder(auth, storage, {
      ...
      manageUsers:
        isLastSegment && createAsSharedFolder ? manageUsers : undefined,
      manageRecords:
        isLastSegment && createAsSharedFolder ? manageRecords : undefined,
