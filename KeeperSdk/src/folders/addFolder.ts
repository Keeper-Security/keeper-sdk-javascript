...
function resolveParentContext(storage: InMemoryStorage, parentUid: string | null): ParentContext {
    ...
    if (sff) {
        return { kind: 'shared_folder_folder', sharedScopeUid: sff.sharedFolderUid }
    }
...
function decideNewFolderType(
    ...
    if (isSharedFolder) {
        if (parent.kind !== 'user_folder' && parent.kind !== 'virtual_root') {
            throw new KeeperSdkError('Shared folders cannot be nested inside other shared folders.', 'shared_folder_nested')
        }
...
async function getEncryptionKeyForNewFolder(
    ...
    if (folderType === 'shared_folder_folder') {
        ...
        if (!k) {
            throw new KeeperSdkError('Shared folder encryption key not available. Sync the vault and try again.', 'shared_folder_key_missing')
        }
        return k
    }
    if (!auth.dataKey) {
...
export async function mkdir(
    ...
    let lastResult: AddFolderResult = { folderUid: '', success: false, message: 'not run' }
    ...
    for (let i = 0; i < segments.length; i++) {
        ...
        if (isShared && parentCtx.kind !== 'user_folder' && parentCtx.kind !== 'virtual_root') {
            throw new KeeperSdkError('Shared folders can only be created under a personal folder.', 'shared_folder_invalid_parent')
        }
        ...
        if (!lastResult.success) {
            return { folderUid: lastResult.folderUid, success: false, message: lastResult.message }
        }
