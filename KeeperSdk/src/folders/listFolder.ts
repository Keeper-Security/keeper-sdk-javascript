...
function buildRecordFlags(
  record: DRecord,
  metadata: DRecordMetadata | undefined,
): string {
...
async function buildFolderParentMap(
  ...
  const childToParent = new Map<
    string,
    { uid: string; kind: VaultStorageKind }
  >();
  ...
  for (const kind of parentKinds) {
    for (const candidate of storage.getAll<{ uid: string } & VaultStorageData>(
      kind,
    )) {
...
export async function listVaultRootFolders(storage: InMemoryStorage): Promise<{
  ...
  for (const sharedFolder of storage.getAll<DSharedFolder>(
    FolderKind.SharedFolder,
  )) {
    ...
    if (
      parent &&
      (parent.kind === FolderKind.SharedFolder ||
        parent.kind === FolderKind.SharedFolderFolder)
    ) {
      continue;
    }
    seen.add(sharedFolder.uid);
...
export function findFolderUidByNameOrUid(
  ...
  if (
    ...
    storage.getByUid<DSharedFolder>(
      FolderKind.SharedFolder,
      trimmedNameOrUid,
    ) ||
    storage.getByUid<DSharedFolderFolder>(
  ...
  for (const userFolder of storage.getAll<DUserFolder>(FolderKind.UserFolder)) {
    if (userFolderName(userFolder).toLowerCase() === lowerNameOrUid)
      return userFolder.uid;
  }
  for (const sharedFolder of storage.getAll<DSharedFolder>(
    FolderKind.SharedFolder,
  )) {
    if (sharedFolderName(sharedFolder).toLowerCase() === lowerNameOrUid)
      return sharedFolder.uid;
  }
  for (const sharedFolderFolder of storage.getAll<DSharedFolderFolder>(
    FolderKind.SharedFolderFolder,
  )) {
    if (
      sharedFolderFolderName(sharedFolderFolder).toLowerCase() ===
      lowerNameOrUid
    )
      return sharedFolderFolder.uid;
  }
  return undefined;
}
...
export async function listFolder(
  ...
  folderRows.sort((rowA, rowB) =>
    rowA.name.localeCompare(rowB.name, undefined, { sensitivity: "base" }),
  );
  recordRows.sort((rowA, rowB) =>
    rowA.name.localeCompare(rowB.name, undefined, { sensitivity: "base" }),
  );
