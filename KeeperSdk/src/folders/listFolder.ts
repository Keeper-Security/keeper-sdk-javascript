import type { DRecord, DRecordMetadata, DSharedFolder, DSharedFolderFolder, DUserFolder } from '@keeper-security/keeperapi'
...
export type ListFolderResult =
    | { detail: true; folders: ListFolderFolderDetail[]; records: ListFolderRecordDetail[] }
    | { detail: false; folders: ListFolderFolderSimple[]; records: ListFolderRecordSimple[] }
...
async function getUserFolderParentMap(storage: InMemoryStorage): Promise<Map<string, string>> {
...
async function countFolderChildren(storage: InMemoryStorage, uid: string): Promise<{ records: number; subfolders: number }> {
...
export async function listFolder(storage: InMemoryStorage, options: ListFolderOptions = {}): Promise<ListFolderResult> {
    ...
    const deps = parentKey === null ? recordDepsAtRoot : (await storage.getDependencies(parentKey)) || []
    const rootUserFolders = parentKey === null ? await listRootUserFolders(storage) : []
    ...
    for (const d of deps) {
        if (d.kind === 'user_folder' && showFolders && parentKey !== null) {
            ...
            folderRows.push({ uid: sff.uid, name, folderKind: 'shared_folder_folder' })
