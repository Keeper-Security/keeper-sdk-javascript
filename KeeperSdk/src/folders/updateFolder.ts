...
function resolveFolderEntity(
  ...
  if (sharedFolder)
    return { kind: FolderKind.SharedFolder, folder: sharedFolder };
  const sharedFolderFolder = storage.getByUid<DSharedFolderFolder>(
    FolderKind.SharedFolderFolder,
    folderUid,
  );
  if (sharedFolderFolder)
    return { kind: FolderKind.SharedFolderFolder, folder: sharedFolderFolder };
  return undefined;
}
...
export async function updateFolder(
  ...
  if (
    resolved.kind === FolderKind.UserFolder ||
    resolved.kind === FolderKind.SharedFolderFolder
  ) {
    if (!trimmedName) {
      throw new KeeperSdkError(
  ...
  const effectiveName =
    resolved.kind === FolderKind.SharedFolder
      ? trimmedName || resolved.folder.name || undefined
      : trimmedName;
  ...
  if (resolved.kind === FolderKind.SharedFolder) {
    ...
    request.name = await encryptForStorage(
      platform.stringToBytes(displayName),
      folderKey,
    );
    ...
    request.can_edit = isBoolean(input.canEdit)
      ? input.canEdit
      : sharedFolder.defaultCanEdit;
    request.can_share = isBoolean(input.canShare)
      ? input.canShare
