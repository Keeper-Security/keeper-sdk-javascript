...
function findUserFolder(
  storage: InMemoryStorage,
  nameOrUid: string,
): DUserFolder | undefined {
  return findByUidOrName(
...
function findSharedFolderFolder(
  storage: InMemoryStorage,
  nameOrUid: string,
): DSharedFolderFolder | undefined {
  return findByUidOrName(
...
async function formatRegularFolder(
  ...
  if (!isUser) {
    base.shared_folder_scope_uid = (
      folder as DSharedFolderFolder
    ).sharedFolderUid;
  }
...
function formatSharedFolder(
  ...
  const base: GetFolderResultSharedFolder = {
    ...
    record_permissions: recordPermissions.length
      ? recordPermissions
      : undefined,
...
function normalizeFormat(
  format: GetFolderFormatInput | undefined,
): GetFolderFormat {
...
export async function getFolder(
  ...
  if (!trimmed) {
    throw new KeeperSdkError(
      "Folder UID or name is required.",
      "missing_folder_ref",
    );
  }
  ...
  if (!found) {
    throw new KeeperSdkError(
      `"${trimmed}" not found as a folder or shared folder`,
      "folder_not_found",
    );
  }
