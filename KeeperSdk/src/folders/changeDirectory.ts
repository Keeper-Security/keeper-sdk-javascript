...
export async function findParentFolderUid(
  ...
  const rootFolderUids = new Set(
    (await listRootUserFolders(storage)).map((folder) => folder.uid),
  );
...
function findChildFolder(
  ...
  const exactNameMatch = children.find(
    (child) => child.name.trim() === trimmedComponent,
  );
  if (exactNameMatch) return exactNameMatch;
  const lowerComponent = trimmedComponent.toLowerCase();
  return children.find(
    (child) => child.name.trim().toLowerCase() === lowerComponent,
  );
}
...
export async function changeDirectory(
  ...
  session.workingFolderDisplayPath = await buildWorkingFolderDisplayPath(
    storage,
    resolved.folderUid,
  );
  return {
...
export function getWorkingFolderDisplayName(
  ...
  return entry?.name
    ? `${VAULT_ROOT_DISPLAY_NAME}/${entry.name}`
    : VAULT_ROOT_DISPLAY_NAME;
}
