...
function getFolderEntryByUid(
    ...
    if (sf) return { uid: sf.uid, name: sharedFolderName(sf), folderKind: 'shared_folder' }
    const sff = storage.getByUid<DSharedFolderFolder>('shared_folder_folder', uid)
    if (sff) return { uid: sff.uid, name: sharedFolderFolderName(sff), folderKind: 'shared_folder_folder' }
    return undefined
}
...
async function listRootUserFolderUids(storage: InMemoryStorage): Promise<Set<string>> {
    const folders = storage.getAll<DUserFolder>('user_folder')
    const childToParent = await getUserFolderParentMap(storage)
    return new Set(folders.filter((f) => !childToParent.has(f.uid)).map((f) => f.uid))
}
