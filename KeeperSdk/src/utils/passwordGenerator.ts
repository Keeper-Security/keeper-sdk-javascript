...
export const GEN_PASSWORD_ALGORITHMS = [
  "rand",
  "dice",
  "crypto",
  "passphrase",
...
export function generatePasswordFromOptions(
  options?: PasswordGenerationOptions | null,
): string {
  ...
  for (const pos of indexes) {
    if (upper > 0) {
      buffer[pos] = String.fromCharCode(
        "A".charCodeAt(0) + (randoms[pos] % LETTER_COUNT),
      );
      upper--;
    } else if (lower > 0) {
      buffer[pos] = String.fromCharCode(
...
function policyToGenerationOptions(
  ...
  else if (policy["special-min"] !== undefined)
    opts.special = policy["special-min"];
  return opts;
}
...
export class KeeperPasswordGenerator {
  ...
  constructor(
    lengthOrOptions:
      | number
      | PasswordGenerationOptions = DEFAULT_PASSWORD_LENGTH,
  ) {
    this.options =
  ...
  static createFromPolicy(
    ...
    return new KeeperPasswordGenerator(
      policyToGenerationOptions(policy, lengthOverride),
    );
  }
}

function clampPassphraseWordCount(
...
class DicewarePasswordGenerator {
  ...
  generate(): string {
    ...
    for (let i = 0; i < this.rolls; i++)
      words.push(this.vocabulary[randomInt(this.vocabulary.length)]);
...
function syncSha256(data: Uint8Array): Uint8Array {
  ...
  throw new Error(
    "crypto passphrase generation requires Node.js (use rand or passphrase instead)",
  );
}
...
export function resolveGenPasswordAlgorithm(
  ...
  if (!parameters || parameters.length === 0)
    return { algorithm: "rand", error: null };
...
function parsePassphraseGenParameters(parameters: readonly string[]): {
  ...
  if (
    !parameters.length ||
    parameters[0].trim().toLowerCase() !== "passphrase"
  ) {
  ...
  if (idx < extras.length && !/^(true|false)$/i.test(extras[idx].trim())) {
    const token = extras[idx].trim();
    if (token.toLowerCase() === "space" || token.toLowerCase() === "sp")
      separator = " ";
    else if (token.length === 1 && PP_SEPARATOR_CHARACTERS.includes(token))
      separator = token;
    else {
      return {
        options: empty,
  ...
  if (idx < extras.length) {
    return {
      options: empty,
      error: `Unexpected $GEN:passphrase parameter "${extras[idx].trim()}".`,
    };
  }
  return {
    options: { wordCount, separator, capitalize, appendNumber },
    error: null,
  };
}
...
export function generatePassword(
  ...
  const { algorithm, error: algError } =
    resolveGenPasswordAlgorithm(parameters);
  if (algError) return { password: null, error: algError };
  if (!algorithm)
    return { password: null, error: "Unknown password generation algorithm." };
  ...
  try {
    if (algorithm === "crypto") {
      if (typeof process === "undefined" || !process.versions?.node) {
        return {
          password: null,
          error:
            "$GEN:crypto is not supported in the browser; use rand or passphrase.",
        };
      }
      return {
        password: new CryptoPassphraseGenerator().generate(),
        error: null,
      };
    }
      ...
      if (policy?.["passphrase-allow"] === false) {
        return {
          password: generatePasswordFromOptions(
            randomPasswordOptions(length, policy),
          ),
          error: null,
        };
      }
    ...
    if (algorithm === "dice") {
      ...
      return {
        password: new DicewarePasswordGenerator(
          rolls,
          DICEWARE_WORDS,
        ).generate(),
        error: null,
...
export function parseGeneratePasswordFlag(
