...
export class KeeperPasswordGenerator {
    ...
    constructor(lengthOrOptions: number | PasswordGenerationOptions = DEFAULT_PASSWORD_LENGTH) {
        this.options = typeof lengthOrOptions === 'number' ? { length: lengthOrOptions } : { ...lengthOrOptions }
    }
    ...
    static createFromPolicy(policy: PasswordComplexityPolicy, lengthOverride?: number | null): KeeperPasswordGenerator {
...
export function resolveGenPasswordAlgorithm(parameters?: readonly string[] | null): {
    algorithm: GenPasswordAlgorithm | null
    error: string | null
} {
...
export function generatePassword(
    ...
    try {
        ...
        if (algorithm === 'dice') {
            ...
            return {
                password: new DicewarePasswordGenerator(rolls, DICEWARE_WORDS).generate(),
                error: null,
            }
        }
