...
export async function viewTeam(
  ...
  const teamUsers = buildTeamUsers(
    response.users || [],
    response.team_users || [],
    team.team_uid,
  );
...
export function formatTeamView(
  view: TeamView,
  options: FormatTeamViewOptions = {},
  ...
  if (view.team_users && view.team_users.length > 0) {
    rows.push({
      ...
      id: verbose
        ? view.team_users.map((user) => user.enterprise_user_id)
        : undefined,
    });
  }
...
export function teamViewTable(table: FormattedTeamViewTable): string {
  ...
  const idWidth = table.hasIdColumn
    ? expandedRows.reduce(
        (max, row) => Math.max(max, ...row.idLines.map((line) => line.length)),
        0,
      )
    : 0;
...
function resolveTeam(
  ...
  if (!trimmed) {
    throw new KeeperSdkError(
      "Team name or UID is required.",
      ResultCodes.TEAM_REQUIRED,
    );
  }
  ...
  const nameMatches = teams.filter(
    (team) => (team.name || "").trim().toLowerCase() === lowered,
  );
...
function buildTeamUsers(
  ...
  for (const id of memberIds) {
    ...
    if (user && user.username)
      result.push({ enterprise_user_id: id, username: user.username });
  }
  result.sort((a, b) =>
