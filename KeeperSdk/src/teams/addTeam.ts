...
export type TeamRestrictionInput =
  | TeamRestriction
  | `${TeamRestriction}`
  | null
  | undefined;
...
export type AddTeamConfirm = (
  prompt: AddTeamConflictPrompt,
...
function buildTeamsByLowerName(
  teams: EnterpriseTeamRecord[],
): Map<string, EnterpriseTeamRecord[]> {
...
async function sendTeamAdd(
  ...
  const encryptedPrivateKey = await encryptForStorage(
    rsaKeyPair.privateKey,
    teamKeyBytes,
  );
  const encryptedEccPrivateKey = await encryptKey(
    eccKeyPair.privateKey,
    teamKeyBytes,
  );
  const encryptedTeamKeyByDataKey = await encryptForStorage(
    teamKeyBytes,
    dataKey,
  );
...
function finalizeResult(
  ...
  const created = items.filter(
    (item) => item.status === AddTeamStatus.Created,
  ).length;
  const skipped = items.filter(
    (item) => item.status === AddTeamStatus.Skipped,
  ).length;
  const failed = items.filter(
    (item) => item.status === AddTeamStatus.Failed,
  ).length;
  return {
