...
const DELETE_TEAM_INCLUDES: EnterpriseDataInclude[] = [
...
export async function deleteTeams(
  ...
  if (requestedIdentifiers.length === 0) {
    throw new KeeperSdkError(
      "No teams to delete.",
      ResultCodes.NO_TEAMS_TO_DELETE,
    );
  }
...
function finalizeResult(items: DeleteTeamItemResult[]): DeleteTeamResult {
  const deleted = items.filter(
    (item) => item.status === DeleteTeamStatus.Deleted,
  ).length;
  const failed = items.filter(
    (item) => item.status === DeleteTeamStatus.Failed,
  ).length;
  return {
...
export function formatDeleteTeamResult(
...
export function renderDeleteTeamAsciiTable(
  table: FormattedDeleteTeamTable,
): string {
