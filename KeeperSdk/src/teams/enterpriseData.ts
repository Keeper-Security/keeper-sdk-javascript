...
const INCLUDE_TO_ENTITY: Record<
  EnterpriseDataInclude,
  Enterprise.EnterpriseDataEntity
> = {
  ...
  [EnterpriseDataInclude.RolePrivileges]:
    Enterprise.EnterpriseDataEntity.ROLE_PRIVILEGES,
  [EnterpriseDataInclude.ManagedNodes]:
    Enterprise.EnterpriseDataEntity.MANAGED_NODES,
  [EnterpriseDataInclude.RoleEnforcements]:
    Enterprise.EnterpriseDataEntity.ROLE_ENFORCEMENTS,
  ...
  [EnterpriseDataInclude.QueuedTeams]:
    Enterprise.EnterpriseDataEntity.QUEUED_TEAMS,
  [EnterpriseDataInclude.QueuedTeamUsers]:
    Enterprise.EnterpriseDataEntity.QUEUED_TEAM_USERS,
  [EnterpriseDataInclude.UserAliases]:
    Enterprise.EnterpriseDataEntity.USER_ALIASES,
};
...
export class EnterpriseDataManager implements EnterpriseDataManagerApi {
  ...
  public async decryptNodeNames(nodes: EnterpriseNode[]): Promise<number> {
    ...
    for (const node of nodes) {
      ...
      const display = await EnterpriseDataManager.decryptDisplayName(
        node.encrypted_data,
        treeKey,
      );
      if (display) {
    ...
    if (decryptedCount > 0) {
      logger.debug(
        `Decrypted ${decryptedCount} enterprise node name(s) using tree key.`,
      );
    }
  ...
  public static getNodePath(
    ...
    const { omitRoot = true, separator = DEFAULT_NODE_PATH_SEPARATOR } =
      options;
  ...
  private async fetchEnterpriseData(
    ...
    for (let iteration = 0; iteration < MAX_CONTINUATIONS; iteration += 1) {
      ...
      const response = await this.auth.executeRest(
        getEnterpriseDataForUserMessage(request),
      );
      ...
      if (
        ...
        response.continuationToken.length === 0
      )
        break;
  ...
  private async fetchAndDecryptTreeKey(): Promise<Uint8Array | null> {
    ...
    try {
      ...
      logger.debug(
        `enterprise/get_enterprise_data_keys failed: ${extractErrorMessage(err)}`,
      );
      return null;
    }
  ...
  private async decryptTreeKey(
    treeKey: Enterprise.ITreeKey,
    ...
    try {
      switch (keyType) {
        case Enterprise.BackupKeyType.ENCRYPTED_BY_DATA_KEY: {
          ...
          return await platform.aesCbcDecrypt(
            encrypted,
            this.auth.dataKey,
            true,
          );
        ...
        case Enterprise.BackupKeyType.ENCRYPTED_BY_PUBLIC_KEY_ECC: {
          ...
          return await platform.privateDecryptEC(
            encrypted,
            this.auth.eccPrivateKey,
          );
        }
        default:
  ...
  private static async decryptDisplayName(
    encrypted: string,
    treeKey: Uint8Array,
  ): Promise<string> {
    try {
  ...
  private static decodeNodeChunk(bytes: Uint8Array): EnterpriseNode {
    ...
    if (message.parentId != null)
      node.parent_id = EnterpriseDataManager.toNumber(message.parentId);
  ...
  private static decodeUserChunk(bytes: Uint8Array): EnterpriseUser {
    ...
    const user: EnterpriseUser = {
      enterprise_user_id: EnterpriseDataManager.toNumber(
    ...
    if (message.nodeId != null)
      user.node_id = EnterpriseDataManager.toNumber(message.nodeId);
    ...
    if (message.accountShareExpiration != null) {
      user.account_share_expiration = EnterpriseDataManager.toNumber(
  ...
  private static decodeRoleChunk(bytes: Uint8Array): EnterpriseRole {
    ...
    if (message.newUserInherit != null)
      out.new_user_inherit = message.newUserInherit;
    return out;
  }

  private static decodeRolePrivilegeChunk(
    bytes: Uint8Array,
  ...
  private static decodeManagedNodeChunk(
    bytes: Uint8Array,
  ...
  private static decodeRoleEnforcementChunk(
    bytes: Uint8Array,
  ...
  private static decodeTeamChunk(bytes: Uint8Array): EnterpriseTeamRecord {
    ...
    if (message.encryptedTeamKey)
      team.encrypted_team_key = message.encryptedTeamKey;
    return team;
  }
  ...
  private static decodeQueuedTeamChunk(
    bytes: Uint8Array,
    ...
    if (message.encryptedData)
      queuedTeam.encrypted_data = message.encryptedData;
    return queuedTeam;
  }

  private static decodeUserAliasChunk(
    bytes: Uint8Array,
    ...
    switch (entity) {
      case Enterprise.EnterpriseDataEntity.NODES:
        target.nodes = (target.nodes || []).concat(
          EnterpriseDataManager.decodeChunk(
            data,
            EnterpriseDataManager.decodeNodeChunk,
          ),
        );
        break;
      case Enterprise.EnterpriseDataEntity.USERS:
        target.users = (target.users || []).concat(
          EnterpriseDataManager.decodeChunk(
            data,
            EnterpriseDataManager.decodeUserChunk,
          ),
        );
        break;
      case Enterprise.EnterpriseDataEntity.ROLES:
        target.roles = (target.roles || []).concat(
          EnterpriseDataManager.decodeChunk(
            data,
            EnterpriseDataManager.decodeRoleChunk,
          ),
        );
        break;
      case Enterprise.EnterpriseDataEntity.TEAMS:
        target.teams = (target.teams || []).concat(
          EnterpriseDataManager.decodeChunk(
            data,
            EnterpriseDataManager.decodeTeamChunk,
          ),
        );
        break;
      case Enterprise.EnterpriseDataEntity.TEAM_USERS:
        target.team_users = (target.team_users || []).concat(
          EnterpriseDataManager.decodeChunk(
            data,
            EnterpriseDataManager.decodeTeamUserChunk,
          ),
        );
        break;
      case Enterprise.EnterpriseDataEntity.ROLE_USERS:
        target.role_users = (target.role_users || []).concat(
          EnterpriseDataManager.decodeChunk(
            data,
            EnterpriseDataManager.decodeRoleUserChunk,
          ),
        );
        break;
      case Enterprise.EnterpriseDataEntity.ROLE_TEAMS:
        target.role_teams = (target.role_teams || []).concat(
          EnterpriseDataManager.decodeChunk(
            data,
            EnterpriseDataManager.decodeRoleTeamChunk,
          ),
        );
        break;
      case Enterprise.EnterpriseDataEntity.QUEUED_TEAMS:
        target.queued_teams = (target.queued_teams || []).concat(
          EnterpriseDataManager.decodeChunk(
            data,
            EnterpriseDataManager.decodeQueuedTeamChunk,
          ),
        );
        break;
      case Enterprise.EnterpriseDataEntity.QUEUED_TEAM_USERS: {
        ...
        target.queued_team_users = (target.queued_team_users || []).concat(
          flatLinks,
        );
        break;
      }
      case Enterprise.EnterpriseDataEntity.USER_ALIASES:
        target.user_aliases = (target.user_aliases || []).concat(
          EnterpriseDataManager.decodeChunk(
            data,
            EnterpriseDataManager.decodeUserAliasChunk,
          ),
        );
        break;
      case Enterprise.EnterpriseDataEntity.ROLE_PRIVILEGES:
        target.role_privileges = (target.role_privileges || []).concat(
          EnterpriseDataManager.decodeChunk(
            data,
            EnterpriseDataManager.decodeRolePrivilegeChunk,
          ),
        );
        break;
      case Enterprise.EnterpriseDataEntity.MANAGED_NODES:
        target.managed_nodes = (target.managed_nodes || []).concat(
          EnterpriseDataManager.decodeChunk(
            data,
            EnterpriseDataManager.decodeManagedNodeChunk,
          ),
        );
        break;
      case Enterprise.EnterpriseDataEntity.ROLE_ENFORCEMENTS:
        target.role_enforcements = (target.role_enforcements || []).concat(
          EnterpriseDataManager.decodeChunk(
            data,
            EnterpriseDataManager.decodeRoleEnforcementChunk,
          ),
        );
        break;
      default:
