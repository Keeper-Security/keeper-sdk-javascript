...
export async function updateTeams(
  ...
  if (requestedIdentifiers.length === 0) {
    throw new KeeperSdkError(
      "No teams to update.",
      ResultCodes.NO_TEAMS_TO_UPDATE,
    );
  }
...
function resolveOptionalRestriction(
  input: TeamRestrictionInput,
): boolean | undefined {
...
async function sendTeamUpdate(
  auth: Auth,
  payload: TeamUpdateRequestPayload,
): Promise<void> {
...
function finalizeResult(items: UpdateTeamItemResult[]): UpdateTeamResult {
  const updated = items.filter(
    (item) => item.status === UpdateTeamStatus.Updated,
  ).length;
  const failed = items.filter(
    (item) => item.status === UpdateTeamStatus.Failed,
  ).length;
  return {
...
export function formatUpdateTeamResult(
...
export function renderUpdateTeamAsciiTable(
  table: FormattedUpdateTeamTable,
): string {
