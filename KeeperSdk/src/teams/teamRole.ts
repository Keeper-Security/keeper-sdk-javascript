...
function existingRoleTeamLinks(
...
export async function changeTeamRoles(
  ...
  const addRoleIds = (input.addRoles || [])
    .map((r) => r.trim())
    .filter(Boolean);
  const removeRoleIds = (input.removeRoles || [])
    .map((r) => r.trim())
    .filter(Boolean);
  ...
  for (const team of ctx.teams as ResolvedTeam[]) {
    ...
    if (removeRoleIds.length > 0) {
      const rolesToRemove = expandRemoveRoles(
        removeRoleIds,
        ctx.roleTeams,
        team.team_uid,
        ctx.allRoles,
  ...
  try {
    ...
    for (const item of items) {
      if (
        item.status === TeamRoleStatus.Added ||
        item.status === TeamRoleStatus.Removed
      ) {
