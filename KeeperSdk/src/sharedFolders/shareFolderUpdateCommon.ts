import type { Auth, DSharedFolder, KeeperResponse, RestCommand } from '@keeper-security/keeperapi'
import { platform, webSafe64FromBytes } from '@keeper-security/keeperapi'
import { InMemoryStorage } from '../storage/InMemoryStorage'
import { KeeperSdkError } from '../utils'

function sharedFolderUpdateCommand(request: Record<string, unknown>): RestCommand<Record<string, unknown>, KeeperResponse> {
    return {
 ...
 */
export function finalizeSharedFolderUpdateRequest(req: Record<string, unknown>): Record<string, unknown> {
    const out: Record<string, unknown> = { ...req, pt: typeof req.pt === 'string' ? req.pt : '' }
...
export async function sendSharedFolderUpdate(
    ...
    return auth.executeRestCommand(sharedFolderUpdateCommand(finalizeSharedFolderUpdateRequest(request)))
}

export function isSuccessResponse(r: KeeperResponse): boolean {
    return r.result === 'success' || (r as { result_code?: string }).result_code === 'success'
}

/**
 * Re-encrypt the shared folder display name for `shared_folder_update` (same pattern as `folder_update`).
 */
export async function encryptedSharedFolderNameB64(sf: DSharedFolder, folderKey: Uint8Array): Promise<string> {
