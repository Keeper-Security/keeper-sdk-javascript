 ...
 */
export function resolveSharedFolderUids(
  storage: InMemoryStorage,
  refs: string[],
): Set<string> {
  if (refs.some((r) => r.trim() === "*")) {
    return new Set(
      storage.getAll<DSharedFolder>("shared_folder").map((f) => f.uid),
    );
  }
  ...
  if (uids.size === 0) {
    throw new KeeperSdkError(
      "No shared folders matched the given name(s) or UID(s).",
...
function tryResolveOneSharedFolderUid(
  storage: InMemoryStorage,
  nameOrUid: string,
): string | null {
 ...
 */
export function tryResolveTeamUid(
  storage: InMemoryStorage,
  nameOrUid: string,
): string | null {
  const t = nameOrUid.trim();
 ...
 */
export function findRecordUidsInVault(
  storage: InMemoryStorage,
  nameOrUid: string,
): string[] {
  const t = nameOrUid.trim();
 ...
 */
export function listRecordUidsInSharedFolder(
  storage: InMemoryStorage,
