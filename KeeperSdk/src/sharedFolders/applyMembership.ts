...
import {
  ...
  type ShareFolderUserStatus,
} from "./shareFolder";
import {
  buildAccountUidEmailMap,
  resolveUserEmail,
} from "./downloadMembership";
import type {
  MembershipData,
...
function resolveSharedFolder(
  storage: InMemoryStorage,
  entry: MembershipSharedFolder,
): DSharedFolder | undefined {
  ...
  const matches = storage
    ...
    .filter(
      (sharedFolder) =>
        sharedFolderName(sharedFolder).toLowerCase() === lowerPath,
    );
...
function tallyCounts(
  ...
  const addUserSet = new Set(
    addUsers.map((grant) => grant.email.toLowerCase()),
  );
  const updateUserSet = new Set(
    updateUsers.map((grant) => grant.email.toLowerCase()),
  );
  const removeUserSet = new Set(
    removeUsers.map((email) => email.toLowerCase()),
  );
...
async function applyFolderMembership(
  ...
  const userPermissions = permissions.filter((permission) =>
    isValidEmail(permission.name),
  );
  const teamPermissions = permissions.filter(
    (permission) => !isValidEmail(permission.name),
  );
  ...
  for (const permission of userPermissions) {
    ...
    if (!existing) {
      ...
      fullSync &&
      (existing.manageUsers !== wantManageUsers ||
        existing.manageRecords !== wantManageRecords)
    ) {
      updateUsers.push({
        email,
        manageUsers: wantManageUsers,
  ...
  if (fullSync) {
    ...
    if (defaultsChanged) {
      try {
        const permissionResult = await updateSharedFolderPermissions(
          auth,
          storage,
          sharedFolder.uid,
          {
        ...
        if (!permissionResult.success)
          permissionsMessage = permissionResult.message;
      } catch (err) {
  ...
  const missingMessage =
    missingTargets.length > 0
      ? `Could not resolve team(s): ${missingTargets.join(", ")}`
      : undefined;
  const message =
...
async function applyTeamMembership(
  ...
  let resolvedTeamUid = "";
  try {
    const resolved = resolveExistingTeams(
      data.teams || [],
  ...
  const toRemove = fullSync
    ? [...currentMembers].filter((email) => !desired.has(email))
    : [];
  ...
  if (toAdd.length > 0) {
    try {
      ...
      if (result.failed > 0)
        messages.push(`${result.failed} member(s) failed to add`);
    } catch (err) {
  ...
  if (toRemove.length > 0) {
    try {
      ...
      if (result.failed > 0)
        messages.push(`${result.failed} member(s) failed to remove`);
    } catch (err) {
...
export async function applyMembership(
  ...
  const success =
    folders.every((folder) => folder.success) &&
    teamMembership.every((team) => team.success);
