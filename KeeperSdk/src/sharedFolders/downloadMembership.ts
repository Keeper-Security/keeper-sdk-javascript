import type { Auth, DSharedFolder, DSharedFolderTeam, DSharedFolderUser, DUser } from '@keeper-security/keeperapi'
...
function resolveForcedFlag(value: boolean, force: boolean | undefined, restrict: boolean | undefined): boolean {
...
function collectFolderPermissions(
    ...
    if (sharedFolder.ownerAccountUid && !seenUserUids.has(sharedFolder.ownerAccountUid)) {
        ...
        if (ownerEmail && !seenEmails.has(key)) {
            ...
            permissions.push({
                name: ownerEmail,
                manage_users: resolveForcedFlag(true, options.forceManageUsers, options.forceRestrictUsers),
                manage_records: resolveForcedFlag(true, options.forceManageRecords, options.forceRestrictRecords),
            })
        }
    }
    ...
    permissions.sort((permissionA, permissionB) =>
        permissionA.name.toLowerCase().localeCompare(permissionB.name.toLowerCase())
    )
    return permissions
}

function matchesFolderFilter(sharedFolder: DSharedFolder, filter: string | undefined): boolean {
...
async function collectEnterpriseTeams(auth: Auth): Promise<MembershipTeam[]> {
    ...
    for (const team of [...(data.teams || []), ...(data.queued_teams || [])]) {
        ...
        const members = Array.from(membersByTeam.get(team.team_uid) || []).sort((memberA, memberB) =>
    ...
    teams.sort((teamA, teamB) => teamA.name.toLowerCase().localeCompare(teamB.name.toLowerCase()))
    return teams
}
...
export async function downloadMembership(
    ...
    const shared_folders: MembershipSharedFolder[] = storage
        ...
        .filter((sharedFolder) => matchesFolderFilter(sharedFolder, options.folderFilter))
        .map((sharedFolder) => ({
            ...
            permissions: collectFolderPermissions(storage, sharedFolder, emailMap, options),
        }))
        .sort((folderA, folderB) => folderA.path.toLowerCase().localeCompare(folderB.path.toLowerCase()))
