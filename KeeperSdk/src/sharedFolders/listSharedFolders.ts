...
function matchEntity(entityWords: string[], searchWords: string[]): boolean {
  ...
  for (const entityWord of entityWords) {
    for (const searchWord of searchWords) {
      if (
        searchWord.length <= entityWord.length &&
        entityWord.includes(searchWord)
      ) {
        return true;
      }
    }
...
function countTeamsForFolder(
  storage: InMemoryStorage,
  sharedFolderUid: string,
): number {
  return countBySharedFolderUid(
    storage.getAll<DSharedFolderTeam>(VaultObjectKind.SharedFolderTeam),
    sharedFolderUid,
  );
}

function countUsersForFolder(
  storage: InMemoryStorage,
  sharedFolderUid: string,
): number {
  return countBySharedFolderUid(
    storage.getAll<DSharedFolderUser>(VaultObjectKind.SharedFolderUser),
    sharedFolderUid,
  );
}

function countRecordsForFolder(
  storage: InMemoryStorage,
  sharedFolderUid: string,
): number {
  return countBySharedFolderUid(
...
function sharedFolderHasPamUserWithRotation(
  storage: InMemoryStorage,
  sharedFolderUid: string,
): boolean {
  for (const link of storage.getAll<DSharedFolderRecord>(
    VaultObjectKind.SharedFolderRecord,
  )) {
...
export function formatSharedFoldersTable(
  ...
  const outRows: string[][] = rows.map((row, rowIndex) => {
    const uid =
      maxWidth == null
        ? row.shared_folder_uid
        : truncateText(row.shared_folder_uid, maxWidth);
