...
function resolveFolder(
  ...
  if (sharedFolder) {
    return {
      ...
      displayName:
        (sharedFolder.name || sharedFolder.uid).trim() || sharedFolder.uid,
    };
  }
  ...
  for (const candidateSharedFolder of storage.getAll<DSharedFolder>(
    FolderKind.SharedFolder,
  )) {
    if (
      (candidateSharedFolder.name || "").trim().toLowerCase() === lowerNameOrUid
    ) {
      return {
        kind: FolderKind.SharedFolder,
  ...
  for (const candidateSharedFolderFolder of storage.getAll<DSharedFolderFolder>(
    FolderKind.SharedFolderFolder,
  )) {
  ...
  for (const candidateUserFolder of storage.getAll<DUserFolder>(
    FolderKind.UserFolder,
  )) {
...
async function removeFromSharedFolder(
  ...
  try {
    response = await auth.executeRest(
      sharedFolderUpdateV3Message({ sharedFoldersUpdateV3: [updateRequest] }),
    );
  } catch (err) {
    return {
      success: false,
  ...
  const requestOk =
    ...
    innerResponse.status === FolderResultStatus.Success;
...
async function shareWithSharedFolder(
  ...
  const sharedFolder = storage.getByUid<DSharedFolder>(
    FolderKind.SharedFolder,
    resolved.sharedFolderUid,
  );
  if (!sharedFolder) {
    throw new KeeperSdkError(
      `Shared folder "${resolved.sharedFolderUid}" not found.`,
  ...
  for (const sharedFolderUser of storage.getAll<DSharedFolderUser>(
    VaultObjectKind.SharedFolderUser,
  )) {
    if (
      sharedFolderUser.sharedFolderUid === sharedFolder.uid &&
      sharedFolderUser.accountUsername
    ) {
  ...
  for (const email of userEmails) {
    ...
    if (publicKeys.rsaPublicKey) {
      const rsaPublicKeyBase64 = platform.bytesToBase64(
  ...
  for (const addUserStatus of innerResponse?.sharedFolderAddUserStatus || []) {
    ...
    const success =
      status === FolderResultStatus.Success ||
      status === FolderResultStatus.Invited;
    userResults.push({
  ...
  for (const updateUserStatus of innerResponse?.sharedFolderUpdateUserStatus ||
    []) {
    const status =
      updateUserStatus.status || ShareFolderUserResultStatus.Unknown;
    userResults.push({
...
export async function shareFolder(
  ...
  if (!resolved) {
    throw new KeeperSdkError(
      `Folder "${input.folder}" was not found.`,
      "folder_not_found",
    );
  }
...
export async function updateSharedFolderMembership(
  ...
  const sharedFolder = storage.getByUid<DSharedFolder>(
    FolderKind.SharedFolder,
    input.sharedFolderUid,
  );
  if (!sharedFolder) {
    throw new KeeperSdkError(
      `Shared folder "${input.sharedFolderUid}" not found.`,
      "shared_folder_not_found",
    );
  }
  ...
  const removeTeams = [
    ...new Set(
      (input.removeTeams || []).map((uid) => uid.trim()).filter(Boolean),
    ),
  ];
  ...
  if (removeUsers.length > 0)
    updateRequest.sharedFolderRemoveUser = removeUsers;
  if (teamsToAdd.length > 0) updateRequest.sharedFolderAddTeam = teamsToAdd;
  if (teamsToUpdate.length > 0)
    updateRequest.sharedFolderUpdateTeam = teamsToUpdate;
  if (removeTeams.length > 0) {
    updateRequest.sharedFolderRemoveTeam = removeTeams.map((teamUid) =>
      normal64Bytes(teamUid),
    );
  }
  ...
  for (const status of innerResponse?.sharedFolderAddUserStatus || []) {
    ...
    results.push({
      ...
      success:
        value === FolderResultStatus.Success ||
        value === FolderResultStatus.Invited,
      status: value,
    });
  }
  ...
  if (requestOk) {
    const reported = new Set(
      results.map((result) => (result.email || "").toLowerCase()),
    );
    for (const user of [...usersToAdd, ...usersToUpdate]) {
      const email = (user.username || "").trim();
      const key = email.toLowerCase();
