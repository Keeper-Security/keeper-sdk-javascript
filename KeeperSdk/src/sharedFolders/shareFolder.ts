...
function sharedFolderUpdateV3Message(
  ...
  Folder.ISharedFolderUpdateV3ResponseV2
> {
  return {
...
function resolveFolder(
  ...
  for (const candidate of storage.getAll<DSharedFolderFolder>(
    "shared_folder_folder",
  )) {
...
async function removeFromSharedFolder(
  ...
  resolved: Extract<
    ResolvedFolder,
    { kind: "shared_folder" | "shared_folder_folder" }
  >,
  emails: string[],
  ...
  try {
    resp = await auth.executeRest(
      sharedFolderUpdateV3Message({ sharedFoldersUpdateV3: [innerRq] }),
    );
  } catch (err) {
    return {
      success: false,
  ...
  return {
    ...
    message: requestOk
      ? undefined
      : innerResp?.status || "shared_folder_update_v3 failed",
    results: perUser,
  };
}
...
async function shareWithSharedFolder(
  ...
  if (emails.length === 0) {
    throw new KeeperSdkError(
      "Provide at least one user email.",
      "no_emails",
    );
  }
