import type { Auth, DSharedFolder, DSharedFolderRecord, KeeperResponse } from '@keeper-security/keeperapi'
...
function isRecordInSharedFolder(storage: InMemoryStorage, sharedFolderUid: string, recordUid: string): boolean {
    for (const r of storage.getAll<DSharedFolderRecord>('shared_folder_record')) {
        if (r.sharedFolderUid === sharedFolderUid && r.recordUid === recordUid) return true
    }
    return false
}
...
export async function shareFolderRecords(
    ...
    if (!recordTargets || recordTargets.length === 0) {
        return { success: true, message: 'Nothing to do', sharedFolderUids: [], responses: [], movesCount: 0 }
    }
    ...
    if (action === 'remove' && setDefaults) {
        throw new KeeperSdkError('A record target of * is not valid for remove. Remove specific records or use @existing.', 'invalid_remove_star')
    }
    if (setDefaults) {
        if (typeof canShare !== 'boolean' && typeof canEdit !== 'boolean') {
            throw new KeeperSdkError('When using * as a record target, set -s and/or -d to on|off.', 'default_record_perm_missing')
        }
    }
    ...
    for (const sharedFolderUid of sfUids) {
        ...
        if (setDefaults) {
            ...
            if (!r.success) {
                return { success: false, message: r.message, sharedFolderUids: sfUids, responses: allResponses, movesCount }
            }
        }
        ...
        if (action === 'remove') {
            ...
            const removeList = Array.from(uids).map(
                (record_uid) => makeRow(record_uid, sharedFolderUid, false, false)
            )
            ...
            if (!isSuccessResponse(resp)) {
                return { success: false, message: (resp as KeeperResponse).message, sharedFolderUids: sfUids, responses: allResponses, movesCount }
            }
            continue
        }
        ...
        if (action === 'grant' && uids.size > 0) {
            ...
            for (const recordUid of uids) {
                const cur = getSharedFolderRecordPerms(storage, sharedFolderUid, recordUid)
                if (!cur) {
                    throw new KeeperSdkError(
    ...
    return { success: true, sharedFolderUids: sfUids, responses: allResponses, movesCount, message: undefined }
}

export function runShareFolderRecords(
    result: ShareFolderRecordsResult
