...
function parseUserTargetList(userTargets: string[]): {
...
function isUserAlreadyInFolder(
  storage: InMemoryStorage,
  sharedFolderUid: string,
  emailLower: string,
): boolean {
...
export async function shareFolderUsers(
  ...
  if (action === "remove" && parsed.setDefaults) {
    throw new KeeperSdkError(
      "Removing a user * target is not used for remove. Omit * or use folder targets only.",
      "invalid_remove_star",
    );
  }
  ...
  for (const label of parsed.teamUids) {
    ...
    const teamUid =
      tryResolveTeamUid(storage, label) ??
      storage.getByUid<DTeam>("team", label.trim())?.uid;
    if (!teamUid) {
      throw new KeeperSdkError(
  ...
  for (const sharedFolderUid of sfUids) {
    ...
    if (parsed.setDefaults) {
      if (
        typeof manageRecords !== "boolean" &&
        typeof manageUsers !== "boolean"
      ) {
        throw new KeeperSdkError(
          "When using * as a user target, set -p and/or -o to on|off.",
          "default_perm_missing",
        );
      }
      const r = await updateSharedFolderPermissions(
        ...
        {
          manageRecords:
            typeof manageRecords === "boolean" ? manageRecords : null,
    ...
    for (const label of parsed.teamUids) {
      const tid =
        tryResolveTeamUid(storage, label) ??
        (label.trim().length >= 16 ? label.trim() : "");
    ...
    if (action === "remove") {
      ...
      const removeUserRows: unknown[] = Array.from(emails).map((e) =>
        emptyUserRow(e),
      );
      ...
      if (
        removeUserRows.length === 0 &&
        removeTeamRows.length === 0 &&
        !parsed.setDefaults
      ) {
        continue;
      }
    ...
    if (teams.length > 0) {
      throw new KeeperSdkError(
        "Granting or updating a team in a shared folder is not supported in this JavaScript SDK yet.",
        "team_grant_not_implemented",
      );
    }
    ...
    const runOne = async (
      ...
      add: unknown[],
      upd: unknown[],
    ): Promise<ShareFolderUsersResult | null> => {
...
export function runShareFolderUsers(result: ShareFolderUsersResult): {
  ok: boolean;
  message: string;
} {
  if (!result.success) {
  ...
  return {
    ...
    message:
      n > 0
        ? `Completed shared folder user request(s) (${n}).`
        : "No shared_folder_update calls were needed.",
  };
}
