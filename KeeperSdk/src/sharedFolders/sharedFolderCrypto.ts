...
export async function loadUserPublicKeyForSharedFolder(auth: Auth, email: string): Promise<UserKeys> {
...
export async function encryptSharedFolderKeyForUser(
    ...
    if (userKeys.eccPublicKey) {
        const enc = await platform.publicEncryptEC(sharedFolderKey, userKeys.eccPublicKey)
